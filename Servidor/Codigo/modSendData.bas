...
110     For LoopC = 1 To TotalUsers
112         tempIndex = Users(LoopC)

118         If UserList(tempIndex).ConnIDValida Then
                If UserList(tempIndex).flags.Muerto = 0 Or Zona(UserList(tempIndex).ZonaId).Segura = 1 Or (UserList(UserIndex).GuildIndex > 0 And UserList(UserIndex).GuildIndex = UserList(tempIndex).GuildIndex) Then
                    enviaDatos = True
...
104     If Not MapaValido(Map) Then Exit Sub
        ...
        Users = GetUsersPosArea(Map, AreaX, AreaY, TotalUsers)
110     For LoopC = 1 To TotalUsers
112         tempIndex = Users(LoopC)
118                 If UserList(tempIndex).ConnIDValida Then
120                     Call modNetwork.Send(tempIndex, Buffer)
