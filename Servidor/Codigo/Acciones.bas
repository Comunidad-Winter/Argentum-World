     ...
     Select Case UserList(UserIndex).Accion.TipoAccion
         ...
         Case e_AccionBarra.Intermundia
             ...
             If UserList(UserIndex).flags.Muerto = 0 Then
                 ...
                 MapData(Mapa).Tile(X, Y).Particula = e_ParticulasIndex.TpVerde
                 MapData(Mapa).Tile(X, Y).TimeParticula = -1
                 MapData(Mapa).Tile(X, Y).TileExit.Map = UserList(UserIndex).flags.PortalMDestino
                 MapData(Mapa).Tile(X, Y).TileExit.X = UserList(UserIndex).flags.PortalXDestino
                 MapData(Mapa).Tile(X, Y).TileExit.Y = UserList(UserIndex).flags.PortalYDestino
                
                 Call SendToAreaByPos(UserList(UserIndex).flags.PortalM, X, Y, PrepareMessageParticleFXToFloor(X, Y, e_ParticulasIndex.TpVerde, -1))
                
                 Call SendToAreaByPos(UserList(UserIndex).flags.PortalM, X, Y, PrepareMessageLightFXToFloor(X, Y, &HFF80C0, 105))
...
104         If MapData(Map).Tile(X, Y).NpcIndex > 0 Then     'Acciones NPCs
106             TempCharIndex = MapData(Map).Tile(X, Y).NpcIndex
...
319                     UserList(UserIndex).flags.pregunta = 5
                    ...
                    End If
                ElseIf NpcList(TempCharIndex).NPCtype = e_NPCType.AOShop Then
...
324                     Call WriteLocaleMsg(UserIndex, "77", e_FontTypeNames.FONTTYPE_INFOIAO)
                ...
                ElseIf NpcList(TempCharIndex).NPCtype = e_NPCType.AOShopPjs Then
...
332                 Call WriteOpenCrafting(UserIndex, NpcList(TempCharIndex).Craftea)
                ...
                '¿Es un obj?
334         ElseIf MapData(Map).Tile(X, Y).ObjInfo.ObjIndex > 0 Then
336             UserList(UserIndex).flags.TargetObj = MapData(Map).Tile(X, Y).ObjInfo.ObjIndex
        
338             Select Case ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex).OBJType
...
362                     If MapData(Map).Tile(X, Y).ObjInfo.ObjIndex = FOGATA_APAG And UserList(UserIndex).flags.Muerto = 0 Then
...
366         ElseIf MapData(Map).Tile(X + 1, Y).ObjInfo.ObjIndex > 0 Then
368             UserList(UserIndex).flags.TargetObj = MapData(Map).Tile(X + 1, Y).ObjInfo.ObjIndex
        
370             Select Case ObjData(MapData(Map).Tile(X + 1, Y).ObjInfo.ObjIndex).OBJType
...
374         ElseIf MapData(Map).Tile(X + 1, Y + 1).ObjInfo.ObjIndex > 0 Then
376             UserList(UserIndex).flags.TargetObj = MapData(Map).Tile(X + 1, Y + 1).ObjInfo.ObjIndex

378             Select Case ObjData(MapData(Map).Tile(X + 1, Y + 1).ObjInfo.ObjIndex).OBJType
...
382         ElseIf MapData(Map).Tile(X, Y + 1).ObjInfo.ObjIndex > 0 Then
384             UserList(UserIndex).flags.TargetObj = MapData(Map).Tile(X, Y + 1).ObjInfo.ObjIndex

386             Select Case ObjData(MapData(Map).Tile(X, Y + 1).ObjInfo.ObjIndex).OBJType
...
Sub AccionParaPuerta(ByVal Map As Integer, ByVal X As Integer, ByVal Y As Integer, ByVal UserIndex As Integer, Optional ByVal SinDistancia As Boolean)
...
104     puerta = ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex)
...
112         MapData(Map).Tile(X, Y).ObjInfo.ObjIndex = puerta.IndexAbierta
114         Call BloquearPuerta(Map, X, Y, False)

        Else 'Cierra puerta
116         MapData(Map).Tile(X, Y).ObjInfo.ObjIndex = puerta.IndexCerrada
118         Call BloquearPuerta(Map, X, Y, True)

        End If

120     If ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex).Subtipo = 1 Then
122          Call AccionParaPuerta(Map, X - 3, Y + 1, UserIndex, True)
        End If

124     Call modSendData.SendToAreaByPos(Map, X, Y, PrepareMessageObjectCreate(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex, MapData(Map).Tile(X, Y).ObjInfo.Amount, X, Y))
...
132         Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(SND_PUERTA, X, Y))
        End If
134     UserList(UserIndex).flags.TargetObj = MapData(Map).Tile(X, Y).ObjInfo.ObjIndex
...
Sub AccionParaPuertaNpc(ByVal Map As Integer, ByVal X As Integer, ByVal Y As Integer, ByVal NpcIndex As Integer)
        On Error GoTo Handler

        Dim puerta As t_ObjData 'ver ReyarB


100     puerta = ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex)

102     If puerta.Cerrada = 1 Then 'Abre la puerta
104         MapData(Map).Tile(X, Y).ObjInfo.ObjIndex = puerta.IndexAbierta
106         Call BloquearPuerta(Map, X, Y, False)

        Else 'Cierra puerta
108         MapData(Map).Tile(X, Y).ObjInfo.ObjIndex = puerta.IndexCerrada
110         Call BloquearPuerta(Map, X, Y, True)

        End If

112     Call modSendData.SendToAreaByPos(Map, X, Y, PrepareMessageObjectCreate(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex, MapData(Map).Tile(X, Y).ObjInfo.Amount, X, Y))
...
100     'If ObjData(MapData(map).Tile(X, Y).ObjInfo.ObjIndex).OBJType = 8 Then
102     If Len(ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex).texto) > 0 Then
104         Call WriteShowSignal(UserIndex, MapData(Map).Tile(X, Y).ObjInfo.ObjIndex)
        Else
...
112         If Zona(.ZonaId).Lluvia And Lloviendo Then
114             Call WriteConsoleMsg(UserIndex, "Esta lloviendo, no podés encender una fogata aquí.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If

116         If MapData(Map).Tile(X, Y).trigger = e_Trigger.ZonaSegura Or Zona(.ZonaId).Segura = 1 Then
118             Call WriteConsoleMsg(UserIndex, "En zona segura no podés hacer fogatas.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If

120         If MapData(Map).Tile(X - 1, Y).ObjInfo.ObjIndex = FOGATA Or _
               MapData(Map).Tile(X + 1, Y).ObjInfo.ObjIndex = FOGATA Or _
               MapData(Map).Tile(X, Y - 1).ObjInfo.ObjIndex = FOGATA Or _
               MapData(Map).Tile(X, Y + 1).ObjInfo.ObjIndex = FOGATA Then
...
140             If Zona(.ZonaId).Terreno <> Ciudad Then
