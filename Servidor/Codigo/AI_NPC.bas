...
Public Const RANGO_VISION_x As Integer = 11
Public Const RANGO_VISION_y As Integer = 9
...
114           agresor = NameIndex(.flags.AttackedBy)
            ...
            Else
                ...
                Users = GetUsersNpcArea(NpcIndex, TotalUsers)
...
Public Function NPCHasAUserInFront(ByVal NpcIndex As Integer, ByRef UserIndex As Integer) As Boolean
    ...
    UserIndex = MapData(NextPosNPC.Map).Tile(NextPosNPC.X, NextPosNPC.Y).UserIndex
...
114         ElseIf AtacaMelee Then
                ...
                UserIndexFront = MapData(NextPosNPC.Map).Tile(NextPosNPC.X, NextPosNPC.Y).UserIndex
...
108                 Call modSistemaCombate.NpcAtacaNpc(NpcIndex, .TargetNPC, False)
                End If
...
106                 If MapData(.Orig.Map).Tile(X, Y).NpcIndex > 0 And NpcIndex <> MapData(.Orig.Map).Tile(X, Y).NpcIndex Then
                        Dim foundNpc As Integer
108                     foundNpc = MapData(.Orig.Map).Tile(X, Y).NpcIndex
                        
110                     If NpcList(foundNpc).Hostile Then
                        
112                         If NpcList(foundNpc).Target = 0 Then
114                             BuscarNpcEnArea = MapData(.Orig.Map).Tile(X, Y).NpcIndex
                                Exit Function
116                         ElseIf UserList(NpcList(foundNpc).Target).flags.NPCAtacado <> foundNpc Then
118                             BuscarNpcEnArea = MapData(.Orig.Map).Tile(X, Y).NpcIndex
                                Exit Function
                            End If
...
110                    Call modSistemaCombate.NpcAtacaNpc(NpcIndex, .TargetNPC)
                   End If
...
114             Call HeadtoPos(Heading, NextTile)
                ...
                ' Si hay un NPC
116             MoveChar = MapData(NextTile.Map).Tile(NextTile.X, NextTile.Y).NpcIndex
...
120                 Call MoveNpcToSide(MoveChar, Heading)
                ...
                ' Si hay un user
122             MoveChar = MapData(NextTile.Map).Tile(NextTile.X, NextTile.Y).UserIndex
...
102     If Not EnRangoVision(NpcIndex, UserIndex) Then Exit Function 'En rango
        ...
        If Zona(NpcList(NpcIndex).ZonaId).Segura <> Zona(UserList(UserIndex).ZonaId).Segura Then Exit Function 'Estan en distinta zona segura
