...
112     If Zona(UserList(UserIndex).ZonaId).Newbie Then
...
146             If EsGM(UserIndex) Then
                    ...
                    If prevCantidad >= 10000 Then
                        Call SendCharacterEvent(UserIndex, e_EventType.DropGold, "Pos: " & UserList(UserIndex).Pos.Map & "," & UserList(UserIndex).Pos.X & "," & UserList(UserIndex).Pos.Y, iORO, prevCantidad, prevCantidad)
                    End If

148                 If MiObj.ObjIndex = iORO Then
150                     Call LogGM(.Name, "Tiro: " & PonerPuntos(prevCantidad) & " monedas de oro.")

                    Else
152                     Call LogGM(.Name, "Tiro cantidad:" & PonerPuntos(prevCantidad) & " Objeto:" & ObjData(MiObj.ObjIndex).Name)
...
110             obj.Amount = num
                ...
                If ObjData(obj.ObjIndex).Valor * num >= 50000 Then
                    Call SendCharacterEvent(UserIndex, e_EventType.DropItem, ObjData(obj.ObjIndex).Name & " en " & Map & "," & X & "," & Y, obj.ObjIndex, num, ObjData(obj.ObjIndex).Valor * num)
                End If
    
112             If ObjData(obj.ObjIndex).Destruye = 0 Then

                    Dim Suma As Long
                    Suma = num + MapData(.Pos.Map).Tile(X, Y).ObjInfo.Amount
    
                    'Check objeto en el suelo
114                 If MapData(.Pos.Map).Tile(X, Y).ObjInfo.ObjIndex = 0 Or (MapData(.Pos.Map).Tile(X, Y).ObjInfo.ObjIndex = obj.ObjIndex And Suma <= MAX_INVENTORY_OBJS) Then
                      
116                     If Suma > MAX_INVENTORY_OBJS Then
118                         num = MAX_INVENTORY_OBJS - MapData(.Pos.Map).Tile(X, Y).ObjInfo.Amount
                        End If
...
100     MapData(Map).Tile(X, Y).ObjInfo.Amount = MapData(Map).Tile(X, Y).ObjInfo.Amount - num

102     If MapData(Map).Tile(X, Y).ObjInfo.Amount <= 0 Then

            
108         MapData(Map).Tile(X, Y).ObjInfo.ObjIndex = 0
110         MapData(Map).Tile(X, Y).ObjInfo.Amount = 0
...
102         If MapData(Map).Tile(X, Y).ObjInfo.ObjIndex = obj.ObjIndex Then
104             MapData(Map).Tile(X, Y).ObjInfo.Amount = MapData(Map).Tile(X, Y).ObjInfo.Amount + obj.Amount
            Else
110             MapData(Map).Tile(X, Y).ObjInfo.ObjIndex = obj.ObjIndex

112             If ObjData(obj.ObjIndex).VidaUtil <> 0 Then
114                 MapData(Map).Tile(X, Y).ObjInfo.Amount = ObjData(obj.ObjIndex).VidaUtil
                Else
116                 MapData(Map).Tile(X, Y).ObjInfo.Amount = obj.Amount

                End If
                
            End If
            
118         Call modSendData.SendToAreaByPos(Map, X, Y, PrepareMessageObjectCreate(obj.ObjIndex, MapData(Map).Tile(X, Y).ObjInfo.Amount, X, Y))
...
Sub PickObj(ByVal UserIndex As Integer)
        ...
        '¿Hay algun obj?
100     If MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).ObjInfo.ObjIndex > 0 Then

            '¿Esta permitido agarrar este obj?
102         If ObjData(MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).ObjInfo.ObjIndex).Agarrable <> 1 Then
...
110             Y = UserList(UserIndex).Pos.Y
                ...
                If UserList(UserIndex).flags.jugando_captura = 1 Then
                    If Not InstanciaCaptura Is Nothing Then
                        If Not InstanciaCaptura.tomaBandera(UserIndex, MapData(UserList(UserIndex).Pos.Map).Tile(X, Y).ObjInfo.ObjIndex) Then
                            Exit Sub
                        End If
                    End If
                End If
...
112             obj = ObjData(MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).ObjInfo.ObjIndex)
114             MiObj.Amount = MapData(UserList(UserIndex).Pos.Map).Tile(X, Y).ObjInfo.Amount
116             MiObj.ObjIndex = MapData(UserList(UserIndex).Pos.Map).Tile(X, Y).ObjInfo.ObjIndex

                If MiObj.ObjIndex = iORO Then
165                 Call EraseObj(MapData(UserList(UserIndex).Pos.Map).Tile(X, Y).ObjInfo.Amount, UserList(UserIndex).Pos.Map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y)
166                 UserList(UserIndex).Stats.GLD = UserList(UserIndex).Stats.GLD + MiObj.Amount
176                 Call WriteUpdateGold(UserIndex)

                    If MiObj.Amount >= 10000 Then
                        Call SendCharacterEvent(UserIndex, e_EventType.PickGold, "Pos: " & UserList(UserIndex).Pos.Map & "," & X & "," & Y, iORO, MiObj.Amount, MiObj.Amount)
                    End If

118             ElseIf Not MeterItemEnInventario(UserIndex, MiObj) Then
                    'Call WriteConsoleMsg(UserIndex, "No puedo cargar mAs t_Objetos.", e_FontTypeNames.FONTTYPE_INFO)
                Else
            
                    If ObjData(MiObj.ObjIndex).Valor * MiObj.Amount >= 50000 Then
                        Call SendCharacterEvent(UserIndex, e_EventType.PickItem, ObjData(MiObj.ObjIndex).Name & " en " & UserList(UserIndex).Pos.Map & "," & X & "," & Y, MiObj.ObjIndex, MiObj.Amount, ObjData(MiObj.ObjIndex).Valor * MiObj.Amount)
                    End If
                    'Quitamos el objeto
120                 Call EraseObj(MapData(UserList(UserIndex).Pos.Map).Tile(X, Y).ObjInfo.Amount, UserList(UserIndex).Pos.Map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y)
...
322         Case e_OBJType.otDanyoMagico
...
516             Case e_OBJType.otDanyoMagico, e_OBJType.otResistencia
                    'Si esta equipado lo quita
...
530                     .Invent.ResistenciaEqpSlot = Slot
                    ...
                    ElseIf ObjData(.Invent.Object(Slot).ObjIndex).OBJType = e_OBJType.otDanyoMagico Then
...
686                         If MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).trigger = CARCEL Then
...
928                             .Stats.UserSkills(s) = 0
                                .Stats.UserSkillsAssigned(s) = 0
930                         Next s
                        
                            Dim SkillLibres As Integer
                        
932                         SkillLibres = 10
934                         SkillLibres = SkillLibres + (3 * (.Stats.ELV - 1))
                        ...
                        If TargObj.Cerrada = 1 Then
                              ...
                              If TargObj.Llave > 0 Then
                                 If TargObj.clave = obj.clave Then
                                    MapData(UserList(UserIndex).flags.TargetObjMap).Tile(UserList(UserIndex).flags.TargetObjX, UserList(UserIndex).flags.TargetObjY).ObjInfo.ObjIndex _
                                    = ObjData(MapData(UserList(UserIndex).flags.TargetObjMap).Tile(UserList(UserIndex).flags.TargetObjX, UserList(UserIndex).flags.TargetObjY).ObjInfo.ObjIndex).IndexCerrada
                                    UserList(UserIndex).flags.TargetObj = MapData(UserList(UserIndex).flags.TargetObjMap).Tile(UserList(UserIndex).flags.TargetObjX, UserList(UserIndex).flags.TargetObjY).ObjInfo.ObjIndex
                              ...
                              Else
                                 If TargObj.clave = obj.clave Then
                                    MapData(UserList(UserIndex).flags.TargetObjMap).Tile(UserList(UserIndex).flags.TargetObjX, UserList(UserIndex).flags.TargetObjY).ObjInfo.ObjIndex _
                                    = ObjData(MapData(UserList(UserIndex).flags.TargetObjMap).Tile(UserList(UserIndex).flags.TargetObjX, UserList(UserIndex).flags.TargetObjY).ObjInfo.ObjIndex).IndexCerradaLlave
...
1064                 If (MapData(.Pos.Map).Tile(.flags.TargetX, .flags.TargetY).Blocked And FLAG_AGUA) = 0 Then
...
1130                 Call WriteWorkRequestTarget(UserIndex, e_Skill.Herreria)
...
1142                         If Zona(.ZonaId).Segura = 1 Then
1144                             Call WriteConsoleMsg(UserIndex, "No hay Peligro aquí. Es Zona Segura ", e_FontTypeNames.FONTTYPE_INFO)
                                      Exit Sub
    
                                  End If
    
1146                              Call SendToAreaByPos(.Pos.Map, .Pos.X, .Pos.Y, PrepareMessagePlayWave(obj.Snd1, .Pos.X, .Pos.Y))
                                  Exit Sub
                              Else
...
1154                         If Zona(.ZonaId).Segura = 1 Then
1156                             Call WriteConsoleMsg(UserIndex, "No hay Peligro aquí. Es Zona Segura ", e_FontTypeNames.FONTTYPE_INFO)
                                      Exit Sub
    
                                  End If
    
1158                              Call SendToAreaByPos(.Pos.Map, .Pos.X, .Pos.Y, PrepareMessagePlayWave(obj.Snd1, .Pos.X, .Pos.Y))
                                  Exit Sub
                              Else
...
1164             Case e_OBJType.otBarcos
                        ...
                        ' Piratas navegan al nivel 21
                     If .Invent.Object(Slot).ObjIndex <> 199 And .Invent.Object(Slot).ObjIndex <> 200 And .Invent.Object(Slot).ObjIndex <> 197 Then
1166                     If .clase = e_Class.Pirat Then
1168                         If .Stats.ELV < 21 Then
1170                             Call WriteConsoleMsg(UserIndex, "Para recorrer los mares debes ser nivel 21 o superior.", e_FontTypeNames.FONTTYPE_INFO)
                                    Exit Sub
                                End If
                        ' Nivel mínimo 24 para navegar, si no sos pirata
1172                    ElseIf .Stats.ELV < 24 Then
1174                        Call WriteConsoleMsg(UserIndex, "Para recorrer los mares debes ser nivel 24 o superior.", e_FontTypeNames.FONTTYPE_INFO)
                            Exit Sub
                        End If
                    ElseIf .Invent.Object(Slot).ObjIndex = 199 Or .Invent.Object(Slot).ObjIndex = 200 Then
                        If MapData(.Pos.Map).Tile(.Pos.X + 1, .Pos.Y).trigger <> e_Trigger.DETALLEAGUA And MapData(.Pos.Map).Tile(.Pos.X - 1, .Pos.Y).trigger <> e_Trigger.DETALLEAGUA And MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y + 1).trigger <> e_Trigger.DETALLEAGUA And MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y - 1).trigger <> e_Trigger.DETALLEAGUA Then
                    ...
                    ElseIf .Invent.Object(Slot).ObjIndex = 197 Then
                          If MapData(.Pos.Map).Tile(.Pos.X + 1, .Pos.Y).trigger <> e_Trigger.NADOCOMBINADO And MapData(.Pos.Map).Tile(.Pos.X - 1, .Pos.Y).trigger <> e_Trigger.NADOCOMBINADO And MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y + 1).trigger <> e_Trigger.NADOCOMBINADO And MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y - 1).trigger <> e_Trigger.NADOCOMBINADO And MapData(.Pos.Map).Tile(.Pos.X + 1, .Pos.Y).trigger <> e_Trigger.VALIDONADO And MapData(.Pos.Map).Tile(.Pos.X - 1, .Pos.Y).trigger <> e_Trigger.VALIDONADO And MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y + 1).trigger <> e_Trigger.VALIDONADO And MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y - 1).trigger <> e_Trigger.VALIDONADO Then
...
1176                If .flags.Navegando = 0 Then
                        If LegalWalk(.Pos.Map, .Pos.X - 1, .Pos.Y, e_Heading.WEST, True, False) Or LegalWalk(.Pos.Map, .Pos.X, .Pos.Y - 1, e_Heading.NORTH, True, False) Or LegalWalk(.Pos.Map, .Pos.X + 1, .Pos.Y, e_Heading.EAST, True, False) Or LegalWalk(.Pos.Map, .Pos.X, .Pos.Y + 1, e_Heading.SOUTH, True, False) Then
                            Call DoNavega(UserIndex, obj, Slot)
                        Else
                            Call WriteConsoleMsg(UserIndex, "¡Debes aproximarte al agua para usar el barco o traje de baño!", e_FontTypeNames.FONTTYPE_INFO)
                        End If
                    ...
                    Else
                        If .Invent.BarcoObjIndex <> .Invent.Object(Slot).ObjIndex Then
                            Call DoNavega(UserIndex, obj, Slot)
                        Else
                            If LegalWalk(.Pos.Map, .Pos.X - 1, .Pos.Y, e_Heading.WEST, False, True) Or LegalWalk(.Pos.Map, .Pos.X, .Pos.Y - 1, e_Heading.NORTH, False, True) Or LegalWalk(.Pos.Map, .Pos.X + 1, .Pos.Y, e_Heading.EAST, False, True) Or LegalWalk(.Pos.Map, .Pos.X, .Pos.Y + 1, e_Heading.SOUTH, False, True) Then
                                Call DoNavega(UserIndex, obj, Slot)
                            Else
                                Call WriteConsoleMsg(UserIndex, "¡Debes aproximarte a la costa para dejar la barca!", e_FontTypeNames.FONTTYPE_INFO)
                            End If
                        End If
                    End If
...
1204                If Zona(.ZonaId).Terreno = "DUNGEON" Then
...
1218                        If MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).trigger = CARCEL Then
...
1314                If MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).trigger = CARCEL Then
1316                    Call WriteConsoleMsg(UserIndex, "No podes usar la runa estando en la carcel.", e_FontTypeNames.FONTTYPE_INFO)
                           Exit Sub
    
                       End If
                        
1318                If Zona(.ZonaId).Segura = 0 And .flags.Muerto = 0 Then
