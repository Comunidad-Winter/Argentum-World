...
118     If tUser > 0 Then
        ...
        Call SendBanCharacter(BannerIndex, UserName, Razon)
...
110     'Call SaveBanDatabase(UserName, Razon, "el sistema")
        Call SendBanCharacter(0, UserName, "Ban automático (Posible BOT)")
...
114     Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg("Servidor Â» Ha baneado a " & UserName & " debido a: " & LCase$(Razon) & ".", e_FontTypeNames.FONTTYPE_SERVER))
...
'
'        ' Le buchoneamos al mundo.
''114     Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg("Servidor Â» " & UserList(BannerIndex).Name & " ha baneado la cuenta de " & UserName & " debido a: " & Reason & ".", e_FontTypeNames.FONTTYPE_SERVER))
'
