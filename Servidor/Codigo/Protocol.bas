...
On Error Resume Next
    ...
    If UserList(UserIndex).Counters.PacketCount > 100 Then
        'Lo kickeo
        Call SendData(SendTarget.ToAdmins, UserIndex, PrepareMessageConsoleMsg("Control de paquetes -> El usuario " & UserList(UserIndex).Name & " | IteraciÛn paquetes | √öltimo paquete: " & PacketId & ".", e_FontTypeNames.FONTTYPE_FIGHT))
        UserList(UserIndex).Counters.PacketCount = 0
    ...
    If PacketId < 0 Or PacketId >= ClientPacketID.PacketCount Then
        ...
        End If
        Call SendData(SendTarget.ToGM, UserIndex, PrepareMessageConsoleMsg("EL USUARIO " & UserList(UserIndex).Name & " | IP: " & UserList(UserIndex).IP & " EST√Å ENVIANDO PAQUETES INV√ÅLIDOS", e_FontTypeNames.FONTTYPE_GUILD))
        Call CloseSocket(UserIndex)
        Exit Function
    End If
...
Private Sub HandleCreateAccount(ByVal UserIndex As Integer)
    ...
    FirstName = Reader.ReadString8
    
    If (LenB(UserName) = 0 Or Len(Password) <> 32) Then
        Call WriteErrorMsg(UserIndex, "Parametros incorrectos")
        Call CloseSocket(UserIndex)
        Exit Sub
    End If
    
    If Len(FirstName) > 30 Or LenB(FirstName) < 2 Or Len(FirstName) > 30 Or Len(FirstName) < 2 Then
        FirstName = "Jugador"
    End If
    
    Call SendCreateAccount(UserIndex, UserName, Password, FirstName, LastName)
...
Private Sub HandleLoginAccount(ByVal UserIndex As Integer)
    ...
    If (UserName = "" Or Password = "" Or LenB(Password) <= 3) Then
        Call WriteShowMessageBox(UserIndex, "Parametros incorrectos")
        Call CloseSocket(UserIndex)
        Exit Sub
    End If
    If Version <> ULTIMAVERSION Then
        Call WriteShowMessageBox(UserIndex, "Parece que tienes una versiÛn desactualizada del cliente, por favor actualizala y vuelve a intentarlo.")
        Call CloseSocket(UserIndex)
        Exit Sub
    End If
...
104         For adminIdx = 0 To AdministratorAccounts.Count - 1
                ' Si el e-mail est· declarado junto al nick de la cuenta donde esta el PJ GM en el Server.ini te dejo entrar.
106             If UCase$(AdministratorAccounts.Items(adminIdx)) = UCase$(UserName) Then
...
128     If Not CheckMailString(UserName) Then
130         Call WriteShowMessageBox(UserIndex, "Email inv·lido.")
            Call CloseSocket(UserIndex)
            Exit Sub
        End If
    
    Call SendLoginAccount(UserIndex, UserName, Password)
...
136         Call CloseSocket(UserIndex)
        ...
        Call ConnectNewUser(UserIndex, Name, race, gender, Class, Head, Hogar)
...
130                     If .Counters.SpeedHackCounter > MaximoSpeedHack Then
                            'Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg("AdministraciÛn ¬ª Posible uso de SpeedHack del usuario " & .name & ".", e_FontTypeNames.FONTTYPE_SERVER))
...
110                 Call WriteConsoleMsg(UserIndex, "No podÈs usar asÌ esta arma.", e_FontTypeNames.FONTTYPE_INFOIAO)
                    Exit Sub
...
108             Call WriteConsoleMsg(UserIndex, "No podÈs tomar ningun objeto.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
134                         Call WriteConsoleMsg(UserIndex, "AcciÛn no permitida.", e_FontTypeNames.FONTTYPE_INFO)
                            Exit Sub
...
146                         Call WriteConsoleMsg(UserIndex, "AcciÛn no permitida.", e_FontTypeNames.FONTTYPE_INFO)
                            Exit Sub
                        End If
...
150                     Call WriteConsoleMsg(UserIndex, "Para tirar la barca deberias estar en tierra firme.", e_FontTypeNames.FONTTYPE_INFO)
                ...
                Else
                    ...
                    Call LogEdicionPaquete("El usuario " & UserList(UserIndex).Name & " editÛ el slot del inventario | Valor: " & Slot & ".")
                End If
...
Private Function verifyTimeStamp(ByVal ActualCount As Long, ByRef LastCount As Long, ByRef LastTick As Long, ByRef Iterations, ByVal UserIndex As Integer, ByVal PacketName As String, Optional ByVal DeltaThreshold As Long = 100, Optional ByVal MaxIterations As Long = 5, Optional ByVal CloseClient As Boolean = False) As Boolean
    ...
    If ActualCount <= LastCount Then
        ...
        Call LogEdicionPaquete("El usuario " & UserList(UserIndex).Name & " editÛ el paquete " & PacketName & ".")
    ...
    If Delta < DeltaThreshold Then
        ...
        If Iterations >= MaxIterations Then
            'Call WriteShowMessageBox(UserIndex, "Relajate and· a tomarte un tÈ con Gulfas.")
            verifyTimeStamp = False
            'Call LogMacroServidor("El usuario " & UserList(UserIndex).name & " iterÛ el paquete " & PacketName & " " & MaxIterations & " veces.")
            Call SendData(SendTarget.ToAdmins, UserIndex, PrepareMessageConsoleMsg("Control de macro---> El usuario " & UserList(UserIndex).Name & "| Revisar --> " & PacketName & " (EnvÌos: " & MaxIterations & ").", e_FontTypeNames.FONTTYPE_INFOBOLD))
...
102         Spell = Reader.ReadInt8()
           ...
           ' client_packet_crc = Reader.ReadInt64
            ...
            '        Call WriteShowMessageBox(UserIndex, "Se te cerrÛ por GordonSui")
            '        Call CloseSocket(UserIndex)
            '    End If
            'End If
...
102         X = Reader.ReadInt16()
104         Y = Reader.ReadInt16()
...
120                         Call WriteConsoleMsg(UserIndex, "No podÈs ocultarte si est·s montado.", e_FontTypeNames.FONTTYPE_INFO)
...
134                     Call WriteConsoleMsg(UserIndex, "No podÈs ocultarte durante un reto.", e_FontTypeNames.FONTTYPE_INFO)
                        Exit Sub

                    End If
                    
136                 If .flags.EnConsulta Then
138                     Call WriteConsoleMsg(UserIndex, "No podÈs ocultarte si estas en consulta.", e_FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
    
                    End If
                    
                    If .flags.invisible Then
139                     Call WriteConsoleMsg(UserIndex, "No podÈs ocultarte si est·s invisible.", e_FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
    
                    End If
                    
140                 If Zona(.ZonaId).SinInvi Then
...
102         Slot = Reader.ReadInt8()
            ...
            If Not DesdeInventario Then
                Call SendData(SendTarget.ToAdmins, UserIndex, PrepareMessageConsoleMsg("El usuario " & .Name & " est· tomando pociones con click estando en hechizos... raaaaaro, poleeeeemico. BAN? Preg a HarThaoS.", e_FontTypeNames.FONTTYPE_INFOBOLD))
            End If
...
166                             Call WriteConsoleMsg(UserIndex, "No tenÈs municiones.", e_FontTypeNames.FONTTYPE_INFO)
...
180                     Call WriteLocaleMsg(UserIndex, "93", e_FontTypeNames.FONTTYPE_INFO)
                        ' Call WriteConsoleMsg(UserIndex, "EstÌs muy cansado para luchar.", e_FontTypeNames.FONTTYPE_INFO)
...
196                         Call WriteLocaleMsg(UserIndex, "8", e_FontTypeNames.FONTTYPE_INFO)
                            'Call WriteConsoleMsg(UserIndex, "EstÌs demasiado lejos para atacar.", e_FontTypeNames.FONTTYPE_WARNING)
...
202                         Call WriteConsoleMsg(UserIndex, "°No podÈs atacarte a vos mismo!", e_FontTypeNames.FONTTYPE_INFO)
...
264             Case e_Skill.magia
                    ...
                    '  If Zona(.zonaId).MagiaSinEfecto > 0 Then
                    ' Call WriteConsoleMsg(UserIndex, "Una fuerza oscura te impide canalizar tu energÌa", e_FontTypeNames.FONTTYPE_FIGHT)
                    '  Exit Sub
                    ' End If
...
410                         If Zona(UserList(UserIndex).ZonaId).Segura = 1 Then
...
416                         If MapData(.Pos.Map).Tile(X, Y).ObjInfo.Amount <= 0 Then
...
424                         DummyInt = MapData(.Pos.Map).Tile(X, Y).ObjInfo.ObjIndex
...
436                                 Call WriteConsoleMsg(UserIndex, "No podÈs quitar raices allÌ.", e_FontTypeNames.FONTTYPE_INFO)
...
444                                 Call DoRaices(UserIndex, X, Y)
                            ...
                            Else
446                             Call WriteConsoleMsg(UserIndex, "No hay ning˙n ·rbol ahÌ.", e_FontTypeNames.FONTTYPE_INFO)
...
502                 If Zona(.ZonaId).Segura = 0 Then
...
524                                     Call WriteLocaleMsg(UserIndex, "8", e_FontTypeNames.FONTTYPE_INFO)
                                        'Call WriteConsoleMsg(UserIndex, "EstÌs demasiado lejos.", e_FontTypeNames.FONTTYPE_INFO)
526                                     Call WriteWorkRequestTarget(UserIndex, 0)
                                    ...
                                    '17/09/02
                                    'Check the trigger
528                                 If MapData(UserList(tU).Pos.Map).Tile(UserList(tU).Pos.X, UserList(tU).Pos.Y).trigger = e_Trigger.ZonaSegura Then
530                                     Call WriteConsoleMsg(UserIndex, "No podÈs robar aquÌ.", e_FontTypeNames.FONTTYPE_WARNING)
532                                     Call WriteWorkRequestTarget(UserIndex, 0)
                                        Exit Sub

                                    End If
                                 
534                                 If MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).trigger = e_Trigger.ZonaSegura Then
536                                     Call WriteConsoleMsg(UserIndex, "No podÈs robar aquÌ.", e_FontTypeNames.FONTTYPE_WARNING)
...
544                         Call WriteWorkRequestTarget(UserIndex, 0)
                    ...
                    Else
546                     Call WriteConsoleMsg(UserIndex, "°No podÈs robar en zonas seguras!", e_FontTypeNames.FONTTYPE_INFO)
...
600                         Call WriteConsoleMsg(UserIndex, "AhÌ no hay ninguna fragua.", e_FontTypeNames.FONTTYPE_INFO)
...
608                     Call WriteConsoleMsg(UserIndex, "AhÌ no hay ninguna fragua.", e_FontTypeNames.FONTTYPE_INFO)
...
618                 tU = .flags.TargetUser
                    ...
                    'Call WritePreguntaBox(UserIndex, UserList(UserIndex).name & " te invitÛ a unirte a su grupo. øDeseas unirte?")
...
652                             Call InvitarMiembro(UserIndex, tU)
                            Else
654                             Call WriteConsoleMsg(UserIndex, "Tu no podÈs invitar usuarios, debe hacerlo " & UserList(UserList(UserIndex).Grupo.Lider).Name & ".", e_FontTypeNames.FONTTYPE_INFOIAO)
...
664                     Call WriteConsoleMsg(UserIndex, "Servidor ¬ª No perteneces a ning˙n clan.", e_FontTypeNames.FONTTYPE_INFOIAO)
                        Exit Sub
                    End If
...
670                     Call WriteConsoleMsg(UserIndex, "Servidor ¬ª El nivel de tu clan debe ser 3 para utilizar esta opciÛn.", e_FontTypeNames.FONTTYPE_INFOIAO)
                        Exit Sub
                    End If
...
680                     Call WriteConsoleMsg(UserIndex, "Servidor ¬ª No podes marcar a un miembro de tu clan.", e_FontTypeNames.FONTTYPE_INFOIAO)
                        Exit Sub
...
708                 If tU > 0 Then
710                     Call WriteConsoleMsg(UserIndex, "Servidor ¬ª [" & UserList(tU).Name & "] seleccionado.", e_FontTypeNames.FONTTYPE_SERVER)
                    Else
...
118             Call SendData(SendTarget.ToAll, UserIndex, PrepareMessageConsoleMsg(.Name & " ha fundado el clan <" & GuildName & "> de alineaciÛn " & GuildAlignment(.GuildIndex) & ".", e_FontTypeNames.FONTTYPE_GUILD))
...
104         If .flags.Muerto = 1 Then
106             Call WriteConsoleMsg(UserIndex, "°Est·s muerto! SÛlo podÈs usar items cuando est·s vivo.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
108                 Call LogSecurity(.Name & " IP:" & .IP & " tratÛ de hackear los skills.")
...
120             Call LogSecurity(.Name & " IP:" & .IP & " tratÛ de hackear los skills.")
...
130                     .UserSkills(i) = .UserSkills(i) + points(i)
                        .UserSkillsAssigned(i) = .UserSkillsAssigned(i) + points(i)
...
118                     NpcList(.flags.TargetNPC).Mascotas = NpcList(.flags.TargetNPC).Mascotas + 1
            ...
            Else
120             Call SendData(SendTarget.ToPCAliveArea, UserIndex, PrepareMessageChatOverHead("No puedo traer m·s criaturas, mata las existentes!", NpcList(.flags.TargetNPC).Char.charindex, vbWhite))
...
110         If .flags.TargetNPC < 1 Then Exit Sub
            ...
            'ÌEl NPC puede comerciar?
112         If NpcList(.flags.TargetNPC).Comercia = 0 Then
114             Call SendData(SendTarget.ToPCAliveArea, UserIndex, PrepareMessageChatOverHead("No tengo ning˙n interÈs en comerciar.", NpcList(.flags.TargetNPC).Char.charindex, vbWhite))
                Exit Sub
...
138                         Call WriteConsoleMsg(UserIndex, "Este objeto es intransferible, no podÈs venderlo.", e_FontTypeNames.FONTTYPE_TALK)
                            Exit Sub
...
142                         Call WriteConsoleMsg(UserIndex, "No puedes comerciar objetos newbie.", e_FontTypeNames.FONTTYPE_TALK)
                ...
                '     Call WriteConsoleMsg(UserIndex, "No podÈs cambiar tu oferta.", e_FontTypeNames.FONTTYPE_TALK)
                '     Exit Sub
...
148                     Call WriteConsoleMsg(UserIndex, "No podÈs vender tu barco mientras lo est·s usando.", e_FontTypeNames.FONTTYPE_TALK)
                        Exit Sub
...
154                     Call WriteConsoleMsg(UserIndex, "No podÈs vender tu montura mientras la est·s usando.", e_FontTypeNames.FONTTYPE_TALK)
                        Exit Sub
...
114             Call SendData(SendTarget.ToGuildMembers, GuildIndex, PrepareMessagePlayWave(45, NO_3D_SOUND, NO_3D_SOUND))
            Else
116             Call WriteConsoleMsg(UserIndex, "No podÈs expulsar ese personaje del clan.", e_FontTypeNames.FONTTYPE_GUILD)
...
108             Call WriteConsoleMsg(UserIndex, errorStr, e_FontTypeNames.FONTTYPE_GUILD)
            ...
            Else
110             Call WriteConsoleMsg(UserIndex, "Tu solicitud ha sido enviada. Espera prontas noticias del lÌder de " & guild & ".", e_FontTypeNames.FONTTYPE_GUILD)
...
128         If Time = 1 Then
130             UpTimeStr = Time & " dÌa, " & UpTimeStr
            Else
132             UpTimeStr = Time & " dÌas, " & UpTimeStr
...
134         Call WriteConsoleMsg(UserIndex, "Server Online: " & UpTimeStr, e_FontTypeNames.FONTTYPE_INFO)
            ...
            Call WriteConsoleMsg(UserIndex, "Tiempo en lÌnea: " & UpTimeStr & " Record de usuarios en simultaneo: " & RecordUsuarios & ".", e_FontTypeNames.FONTTYPE_INFOIAO)
        End With
...
104             Call WriteConsoleMsg(UserIndex, "No podÈs salir estando paralizado.", e_FontTypeNames.FONTTYPE_WARNING)
                Exit Sub
...
108             Call WriteConsoleMsg(UserIndex, "Primero tenes que seleccionar un personaje, hace click izquierdo sobre Èl.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
112             Call WriteLocaleMsg(UserIndex, "8", e_FontTypeNames.FONTTYPE_INFO)
                'Call WriteConsoleMsg(UserIndex, "EstÌs demasiado lejos del vendedor.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
104             Call WriteConsoleMsg(UserIndex, "°Est·s muerto!", e_FontTypeNames.FONTTYPE_INFO)
            ...
            'Validate target NPC
106         If .flags.TargetNPC = 0 Then
108             Call WriteConsoleMsg(UserIndex, "Primero tenÈs que seleccionar un personaje, hace click izquierdo sobre Èl.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
100     With UserList(UserIndex)
            ...
            'Dead users can't use pets
102         If .flags.Muerto = 1 Then
104             Call WriteConsoleMsg(UserIndex, "°Est·s muerto! Solo podÈs usar items cuando est·s vivo.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
112                 Call WriteConsoleMsg(UserIndex, "Te acomod·s junto a la fogata y comenzÌs a descansar.", e_FontTypeNames.FONTTYPE_INFO)
                Else
...
Private Sub HandleMeditate(ByVal UserIndex As Integer)
        ...
        'ArreglÌ un bug que mandaba un index de la meditacion diferente
...
104         If .clase = e_Class.Hunter Or .clase = e_Class.Warrior Or .clase = e_Class.Pirat Or .clase = e_Class.Thief Then Exit Sub
...
104             Call WriteConsoleMsg(UserIndex, "Primero tenÈs que seleccionar un personaje, hace click izquierdo sobre Èl.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
116         Call WriteConsoleMsg(UserIndex, "ÌÌHas sido curado!", e_FontTypeNames.FONTTYPE_INFO)
...
114                 Call WriteConsoleMsg(UserIndex, "No podÈs vender items.", e_FontTypeNames.FONTTYPE_WARNING)
                    Exit Sub
...
120                     Call WriteChatOverHead(UserIndex, "No tengo ning˙n interÈs en comerciar.", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)
...
132                 Call WriteConsoleMsg(UserIndex, "No podÈs vender items.", e_FontTypeNames.FONTTYPE_WARNING)
                    Exit Sub
...
136                 Call WriteConsoleMsg(UserIndex, "No podÈs vender items a este usuario.", e_FontTypeNames.FONTTYPE_WARNING)
                    Exit Sub
...
140                 Call WriteConsoleMsg(UserIndex, "°No podÈs comerciar con los muertos!", e_FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
...
144                 Call WriteConsoleMsg(UserIndex, "No podÈs comerciar con vos mismo...", e_FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
...
148                 Call WriteLocaleMsg(UserIndex, "8", e_FontTypeNames.FONTTYPE_INFO)
                    'Call WriteConsoleMsg(UserIndex, "EstÌs demasiado lejos del usuario.", e_FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
                End If
...
150             If UserList(.flags.TargetUser).flags.Comerciando = True Then
                    Call FinComerciarUsu(.flags.TargetUser, True)
152                 Call WriteConsoleMsg(UserIndex, "No podÈs comerciar con el usuario en este momento.", e_FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
...
106             Call WriteConsoleMsg(UserIndex, "Primero tenÈs que seleccionar un personaje, hacÈ click izquierdo sobre Èl.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
100     With UserList(UserIndex)
            ...
            'Validate target NPC
102         If .flags.TargetNPC = 0 Then
104             Call WriteConsoleMsg(UserIndex, "Primero tenÈs que seleccionar un personaje, hacÈ click izquierdo sobre Èl.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
118             Call WriteChatOverHead(UserIndex, "Tu deber es combatir criminales, cada 100 criminales que derrotes te darÌ una recompensa.", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)
            
            Else

120             If .Faccion.Status <> e_Facciones.Caos Or .Faccion.Status <> e_Facciones.concilio Then
122                 Call WriteChatOverHead(UserIndex, "No perteneces a la legiÛn oscura!", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)
                    Exit Sub

                End If

124             Call WriteChatOverHead(UserIndex, "Tu deber es sembrar el caos y la desesperanza, cada 100 ciudadanos que derrotes te darÌ una recompensa.", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)
...
114     If Time = 1 Then
116         UpTimeStr = Time & " dÌa, " & UpTimeStr
        Else
118         UpTimeStr = Time & " dÌas, " & UpTimeStr
...
120                      Call SendData(SendTarget.ToDiosesYclan, .GuildIndex, PrepareMessageGuildChat("[CLAN - " & .Name & "] " & chat, .Faccion.Status))
                   ...
                   ' Call SendData(SendTarget.ToAll, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbYellow & "Ì< " & chat & " >Ì" & CStr(UserList(UserIndex).Char.CharIndex))
                End If
...
104         If .GuildIndex <> 0 Then
106             Call WriteConsoleMsg(UserIndex, "CompaÒeros de tu clan conectados: " & onlineList, e_FontTypeNames.FONTTYPE_GUILDMSG)
...
104         If .flags.Muerto = 1 Then
106             Call WriteConsoleMsg(UserIndex, "No podÈs cambiar la descripciÛn estando muerto.", e_FontTypeNames.FONTTYPE_INFOIAO)

            Else
            
108             If Len(Description) > 128 Then
110                 Call WriteConsoleMsg(UserIndex, "La descripciÛn es muy larga.", e_FontTypeNames.FONTTYPE_INFOIAO)

112             ElseIf Not DescripcionValida(Description) Then
114                 Call WriteConsoleMsg(UserIndex, "La descripciÛn tiene car·cteres inv·lidos.", e_FontTypeNames.FONTTYPE_INFOIAO)
                
                Else
116                 .Desc = Trim$(Description)
118                 Call WriteConsoleMsg(UserIndex, "La descripciÛn a cambiado.", e_FontTypeNames.FONTTYPE_INFOIAO)
...
104         If LenB(Name) = 0 Then Exit Sub
            If (.flags.Privilegios And (e_PlayerType.Consejero Or e_PlayerType.SemiDios Or e_PlayerType.Admin Or e_PlayerType.Dios)) = 0 Then
                Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If
...
130             TargetUserName = .Name
            ...
            Call SendGetUserPenalties(UserIndex, TargetUserName)
...
108         ElseIf .flags.TargetNPC = 0 Then
                'Validate target NPC
110             Call WriteConsoleMsg(UserIndex, "Primero tenÈs que seleccionar un personaje, haz click izquierdo sobre Èl.", e_FontTypeNames.FONTTYPE_INFO)
...
118             Call WriteChatOverHead(UserIndex, "No tengo ning˙n interÈs en apostar.", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)

120         ElseIf Amount < 1 Then
122             Call WriteChatOverHead(UserIndex, "El mÌnimo de apuesta es 1 moneda.", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)
...
120             Call SendWithdrawGold(UserIndex, Amount)

            Else
128             Call WriteChatOverHead(UserIndex, "No tenÈs esa cantidad.", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)
...
132                         If Not PersonajeEsLeader(.Name) Then
                                'Me fijo de que alineaciÛn es el clan, si es ARMADA, lo hecho
...
136                                 Call WriteConsoleMsg(UserIndex, "Has dejado el clan.", e_FontTypeNames.FONTTYPE_GUILD)
                                End If
                            Else
                                'Me fijo si est· en un clan armada, en ese caso no lo dejo salir de la facciÛn
                                If GuildAlignmentIndex(.GuildIndex) = e_ALINEACION_GUILD.ALINEACION_ARMADA Then
138                                 Call WriteChatOverHead(UserIndex, "Para dejar la facciÛn primero deber·s ceder el liderazgo del clan", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)
                                    Exit Sub
                                End If
                            End If
                        End If
...
142                     Call WriteChatOverHead(UserIndex, "Ser·s bienvenido a las fuerzas imperiales si deseas regresar.", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)
                        ...
                        Exit Sub
                    Else
144                     Call WriteChatOverHead(UserIndex, "°Sal de aquÌ bufÛn!", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)
                        ...
                        'Si tiene clan
                         If .GuildIndex > 0 Then
                            ...
                            If Not PersonajeEsLeader(.Name) Then
                                'Me fijo de que alineaciÛn es el clan, si es CAOS, lo hecho
                                ...
                                End If
                            Else
                                'Me fijo si est· en un clan CAOS, en ese caso no lo dejo salir de la facciÛn
                                If GuildAlignmentIndex(.GuildIndex) = e_ALINEACION_GUILD.ALINEACION_CAOTICA Then
                                    Call WriteChatOverHead(UserIndex, "Para dejar la facciÛn primero deber·s ceder el liderazgo del clan", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)
                                    Exit Sub
                                End If
                            End If
                        End If
...
162                     Call WriteChatOverHead(UserIndex, "Ya volver·s arrastrandote.", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)
                    Else
164                     Call WriteChatOverHead(UserIndex, "Sal de aquÌ maldito criminal", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)

                    End If

                Else
166                 Call WriteChatOverHead(UserIndex, "°No perteneces a ninguna facciÛn!", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)
...
122             .Stats.GLD = .Stats.GLD - Amount
                Call SendDepositGold(UserIndex, Amount)
...
104             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If
...
112                 Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg(.Name & " ¬ª " & message, e_FontTypeNames.FONTTYPE_GMMSG))
                End If
...
114                     Call WriteConsoleMsg(UserIndex, "Usuario offline.", e_FontTypeNames.FONTTYPE_INFO)
                    Else
116                     Call WriteConsoleMsg(UserIndex, "No podÈs ir cerca de un GM de mayor jerarquÌa.", e_FontTypeNames.FONTTYPE_INFO)
                    End If
                Else
...
126                                 If MapData(UserList(tIndex).Pos.Map).Tile(X, Y).UserIndex = 0 Then
...
148                         Call WriteConsoleMsg(UserIndex, "Todos los lugares est·n ocupados.", e_FontTypeNames.FONTTYPE_INFO)
                        End If
                    Else
150                     Call WriteConsoleMsg(UserIndex, "No podÈs ir cerca de un GM de mayor jerarquÌa.", e_FontTypeNames.FONTTYPE_INFO)
                    End If
                End If
            Else
152             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª No podÈs ir cerca de ningun Usuario si no pidio SOS.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
114                     Call WriteConsoleMsg(UserIndex, "UbicaciÛn  " & UserName & ": " & UserList(tUser).Pos.Map & ", " & UserList(tUser).Pos.X & ", " & UserList(tUser).Pos.Y & ".", e_FontTypeNames.FONTTYPE_INFO)
116                     Call LogGM(.Name, "/Donde " & UserName)
                    End If

                End If
            Else
118             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
120                 Call WriteConsoleMsg(UserIndex, "Solo puedes transportarte a ti mismo.", e_FontTypeNames.FONTTYPE_INFO)
            ...
            'øPara que te vas a transportar a la misma posicion?
...
136             Else
                    ...
                    If UCase$(UserName) <> "YO" Then
                        If .flags.Privilegios And e_PlayerType.Consejero Or e_PlayerType.SemiDios Then
                            If Zona(MapData(Map).Tile(X, Y).ZonaId).Interdimensional = 0 Then
                                Call WriteConsoleMsg(UserIndex, "Solamente puedes teletransportar gente a mapas de eventos o interdimensionales.", e_FontTypeNames.FONTTYPE_INFO)
                                Exit Sub
                            End If
                        End If
...
144                     Call LogGM(.Name, "TransportÛ a " & UserList(tUser).Name & " hacia " & "Mapa" & Map & " X:" & X & " Y:" & Y)
                    End If
...
114                     If CompararPrivilegios(.flags.Privilegios, UserDarPrivilegioLevel(UserName)) > 0 Then
                            ...
                            Call SendSilenceChar(UserIndex, UserName, minutos)


116                         If minutos > 0 Then

122                             Call SendData(SendTarget.ToGM, 0, PrepareMessageConsoleMsg("AdministraciÛn ¬ª " & .Name & " ha silenciado a " & UserName & "(offline) por " & minutos & " minutos.", e_FontTypeNames.FONTTYPE_GM))
124                             Call LogGM(.Name, "Silenciar a " & UserName & " por " & minutos & " minutos.")
                            Else

128                             Call SendData(SendTarget.ToGM, 0, PrepareMessageConsoleMsg("AdministraciÛn ¬ª " & .Name & " ha desilenciado a " & UserName & "(offline).", e_FontTypeNames.FONTTYPE_GM))
130                             Call LogGM(.Name, "Desilenciar a " & UserName & ".")
...
148                     Call SendData(SendTarget.ToGM, 0, PrepareMessageConsoleMsg("AdministraciÛn ¬ª " & .Name & " ha silenciado a " & UserList(tUser).Name & " por " & minutos & " minutos.", e_FontTypeNames.FONTTYPE_GM))
...
154                     UserList(tUser).flags.Silenciado = 0
                        UserList(tUser).flags.MinutosRestantes = 0
                        UserList(tUser).flags.SegundosPasados = 0

156                     Call SendData(SendTarget.ToGM, 0, PrepareMessageConsoleMsg("AdministraciÛn ¬ª " & .Name & " ha desilenciado a " & UserList(tUser).Name & ".", e_FontTypeNames.FONTTYPE_GM))
...
120                 Call WriteConsoleMsg(UserIndex, "Se le ha avisado a " & UserList(tUser).Name & " que quieres ir a su posiciÛn.", e_FontTypeNames.FONTTYPE_INFO)
122                 Call WriteConsoleMsg(tUser, .Name & " quiere transportarse a tu ubicaciÛn. Escribe /sum " & .Name & " para traerlo.", e_FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
...
138             Call LogGM(.Name, "/IRA " & UserName & " Mapa:" & UserList(tUser).Pos.Map & " X:" & UserList(tUser).Pos.X & " Y:" & UserList(tUser).Pos.Y)
            Else
140             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo. solo puedes ir a Usuarios que piden SOS.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
104             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª /TRABAJANDO es un comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If
...
126                         Call WriteConsoleMsg(UserIndex, "No podÈs encarcelar a administradores.", e_FontTypeNames.FONTTYPE_INFO)
                    
128                     ElseIf jailTime > 720 Then
130                         Call WriteConsoleMsg(UserIndex, "No podÈs encarcelar por m·s de 12 horas.", e_FontTypeNames.FONTTYPE_INFO)
                        Else
...
138                             UserName = Replace(UserName, "/", "")
                            ...
                            Call SendAddPenalty(UserIndex, UserName, jailTime, Reason)
                        
152                         Call Encarcelar(tUser, jailTime, .Name)
154                         Call LogGM(.Name, " encarcelÛ a " & UserName)
                ...
                End If
            Else
156             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
108             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If
...
126             UserName = Replace(UserName, "/", "")
            ...
            End If
                ...
                Call SendAddPenalty(UserIndex, UserName, 0, "ADVERTENCIA: " & Reason)
...
152                 Call WriteConsoleMsg(tUser, "TenÈs " & UserList(tUser).Stats.Advertencias & " advertencias actualmente.", e_FontTypeNames.FONTTYPE_CENTINELA)
...
126                 Call LogGM(.Name, "EnviÛ mensaje como GM a " & UserName & ": " & mensaje)
...
152                 If tUser <= 0 Then
156                     Call WriteConsoleMsg(UserIndex, "Usuario offline: " & UserName, e_FontTypeNames.FONTTYPE_INFO)
                    Else
...
168                     Call WriteConsoleMsg(UserIndex, "Usuario offline: " & UserName, e_FontTypeNames.FONTTYPE_INFO)
                    Else
...
178                     Call WriteConsoleMsg(UserIndex, "Usuario offline: " & UserName, e_FontTypeNames.FONTTYPE_INFO)
                    Else
...
188                     Call WriteConsoleMsg(UserIndex, "Usuario offline: " & UserName, e_FontTypeNames.FONTTYPE_INFO)
                    Else
...
202                         Call WriteConsoleMsg(UserIndex, "Usuario offline: " & UserName, e_FontTypeNames.FONTTYPE_INFO)
                        Else
...
214                 If tUser <= 0 Then
218                     Call WriteConsoleMsg(UserIndex, "Usuario offline: " & UserName, e_FontTypeNames.FONTTYPE_INFO)
                    Else
...
236                         UserList(tUser).Stats.CriminalesMatados = MAXUSERMATADOS
                        Else
238                         UserList(tUser).Stats.CriminalesMatados = val(Arg1)
...
250                         UserList(tUser).Stats.ciudadanosMatados = MAXUSERMATADOS
                        Else
252                         UserList(tUser).Stats.ciudadanosMatados = val(Arg1)
...
264                         Call WriteConsoleMsg(UserIndex, "No podÈs tener un nivel superior a " & STAT_MAXELV & ".", FONTTYPE_INFO)
...
300                     If tUser <= 0 Then
304                         Call WriteConsoleMsg(UserIndex, "Usuario offline: " & UserName, e_FontTypeNames.FONTTYPE_INFO)
                        Else
...
316                 If tUser <= 0 Then
320                     Call WriteConsoleMsg(UserIndex, "Usuario offline: " & UserName, e_FontTypeNames.FONTTYPE_INFO)
                    Else
...
498                     UserList(tUser).Desc = Arg1
                    ...
                    Else
500                     Call WriteConsoleMsg(UserIndex, "Caracteres inv·lidos en la descripciÛn.", e_FontTypeNames.FONTTYPE_INFO)
...
118                     Call SendUserStatsTxt(UserIndex, TargetIndex)
                ...
                End If
            Else
120             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
112                 Call WriteConsoleMsg(UserIndex, "Usuario offline.", e_FontTypeNames.FONTTYPE_INFO)
...
128                 Call SendUserSkillsTxt(UserIndex, tUser)
                ...
                End If
            Else
130             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
118                     If .flags.Muerto = 1 Then
                            If UserList(UserIndex).flags.Privilegios And e_PlayerType.SemiDios Then
                                If Zona(.ZonaId).Segura = 0 And Zona(.ZonaId).Interdimensional = 0 Then
                                     Call WriteConsoleMsg(UserIndex, "Servidor ¬ª No puedes revivir en una zona insegura.", e_FontTypeNames.FONTTYPE_INFO)
                                     Exit Sub
                                End If
                            End If
120                         .flags.Muerto = 0
...
134                 Call LogGM(.Name, "Resucito a " & UserName)
                ...
                End If
            Else
136             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
Private Sub HandleSeguirMouse(ByVal UserIndex As Integer)
        ...
        '***************************************************
        'Author: MartÌn Trionfetti - HarThaoS
...
108                 tUser = NameIndex(UserName)
                ...
                ElseIf tUser <> .flags.SigueUsuario And .flags.SigueUsuario > 0 And tUser > 0 Then
                    ...
                    If UserList(tUser).flags.GMMeSigue > 0 And UserList(tUser).flags.GMMeSigue <> UserIndex Then
                        Call WriteConsoleMsg(UserIndex, "El usuario est· siendo seguido por " & UserList(UserList(tUser).flags.GMMeSigue).Name & ".", e_FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    End If
                    ...
                    tempArea = UserList(UserIndex).AreaId
                    ...
                    If tempArea <> UserList(tUser).AreaId Then
                        Call MakeUserChar(UserIndex, tUser, UserList(tUser).Pos.Map, UserList(tUser).Pos.X, UserList(tUser).Pos.Y)
                    End If
                    ...
                    UserList(UserIndex).AreaMap = UserList(tUser).AreaMap
                    UserList(UserIndex).AreaId = UserList(tUser).AreaId
...
End Sub
'HarThaoS: Agrego perdÛn faccionario.
'Puto el que lee
...
110                 tUser = UserIndex
                ...
                If UserList(tUser).Faccion.Status = e_Facciones.Armada Or UserList(tUser).Faccion.Status = e_Facciones.Caos Or UserList(tUser).Faccion.Status = e_Facciones.consejo Or UserList(tUser).Faccion.Status = e_Facciones.concilio Then
                    Call WriteConsoleMsg(UserIndex, "No puedes perdonar a alguien que ya pertenece a una facciÛn", e_FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
                End If
...
114                 Call WriteConsoleMsg(UserIndex, "Usuario offline.", e_FontTypeNames.FONTTYPE_INFO)
                Else
                    ...
                    If UserList(tUser).Faccion.Status = e_Facciones.Ciudadano Then
                        If UserList(tUser).Stats.ciudadanosMatados > 0 Or UserList(tUser).Faccion.Reenlistadas > 0 Then
                            UserList(tUser).Stats.ciudadanosMatados = 0
...
104         If (.flags.Privilegios And (e_PlayerType.Admin Or e_PlayerType.Dios)) Then
                ...
                Else
                    ...
                    If Precio < 0 Then
                        Call WriteConsoleMsg(UserIndex, "El valor de la entrada al evento no podr· ser menor que 0.", e_FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    End If
                
                    'Me fijo si que la cantidad de participantes sea par
                    If cantidad_participantes Mod 2 <> 0 Then
                        Call WriteConsoleMsg(UserIndex, "La cantidad de participantes debe ser un n˙mero par.", e_FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    End If
                    
                    'Permito un m·ximo de 48 participantes
                    If cantidad_participantes > 48 Then 'Leer de una variable de configuraciÛn
                        Call WriteConsoleMsg(UserIndex, "La cantidad de participantes no podr· ser mayor que 48.", e_FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    End If
                    
                    'Me fijo si hay m·s participantes conectados que el cupo para jugar
...
Private Sub HandleParticiparCaptura(ByVal UserIndex As Integer)
    ...
    If InstanciaCaptura Is Nothing Then
        Call WriteConsoleMsg(UserIndex, "Eventos ¬ª No hay ninguna instancia en curso para ese evento.", e_FontTypeNames.FONTTYPE_INFO)
    Else
...
100     With UserList(UserIndex)
            ...
            'Se asegura que el target es un npc
102         If .flags.TargetNPC = 0 Then
104             Call WriteConsoleMsg(UserIndex, "Primero tenÈs que seleccionar al sacerdote.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
128         If .Stats.ciudadanosMatados > 0 Then
                Dim Donacion As Long
130             Donacion = .Stats.ciudadanosMatados * OroMult * CostoPerdonPorCiudadano

132             Call WriteChatOverHead(UserIndex, "Has matado a ciudadanos inocentes, Dios no puede perdonarte lo que has hecho. " & "Pero si haces una generosa donaciÛn de, digamos, " & PonerPuntos(Donacion) & " monedas de oro, tal vez cambie de opiniÛn...", priest.Char.charindex, vbWhite)
                Exit Sub

            Else
                Call WriteChatOverHead(UserIndex, "Para volver a ser un ciudadano deber·s donar 10.000 monedas de oro.", priest.Char.charindex, vbWhite)
            End If
            ...
            If Not permitePerdon Then
                Call WriteChatOverHead(UserIndex, "No podr·s ser perdonado perteneciendo a un clan de alineaciÛn Criminal o de AlineaciÛn Oscura.", priest.Char.charindex, vbYellow)
                Exit Sub
            End If

134         Call WriteChatOverHead(UserIndex, "Con estas palabras, te libero de todo tipo de pecados. °Que Dios te acompaÒe hijo mÌo!", priest.Char.charindex, vbYellow)
...
122                     Call LogGM(.Name, "Echo a " & UserName)
                ...
                End If
            Else
124             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
108             Call BanPJ(UserIndex, UserName, Reason)
            Else
110             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
104         If (.flags.Privilegios And (e_PlayerType.Admin Or e_PlayerType.Dios)) Then
                ...
                Call SendUnbanCharacter(UserIndex, UserName, "")
            Else
122             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
124                 Call WriteConsoleMsg(UserIndex, "Has desplazado a la criatura.", e_FontTypeNames.FONTTYPE_INFO)
                Else
126                 Call WriteConsoleMsg(UserIndex, "SÛlo puedes mover NPCs dentro del mismo mapa.", e_FontTypeNames.FONTTYPE_INFO)
...
130             Call WriteConsoleMsg(UserIndex, "Se le ha avisado a " & UserList(tUser).Name & " que quieres traerlo a tu posiciÛn.", e_FontTypeNames.FONTTYPE_INFO)
132             Call WriteConsoleMsg(tUser, .Name & " quiere transportarte a su ubicaciÛn. Escribe /ira " & .Name & " para ir.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
142                 Call WriteConsoleMsg(tUser, .Name & " te ha trasportado.", e_FontTypeNames.FONTTYPE_INFO)
                ...
                'HarThaoS: Si lo sumonean a un mapa interdimensional desde uno no interdimensional me guardo la posiciÛn de donde viene.
144             If Zona(.ZonaId).Interdimensional = 1 And Zona(UserList(tUser).ZonaId).Interdimensional = 0 Then
...
148             Call WarpToLegalPos(tUser, .Pos.Map, .Pos.X, .Pos.Y + 1, True, True)
                Call WriteConsoleMsg(UserIndex, "Has traÌdo a " & UserList(tUser).Name & ".", e_FontTypeNames.FONTTYPE_INFO)
152             Call LogGM(.Name, "/SUM " & UserName & " Map:" & .Pos.Map & " X:" & .Pos.X & " Y:" & .Pos.Y)
            ...
            End If
        Else
154         Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
        End If
...
106             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            
108         ElseIf .flags.Privilegios And (e_PlayerType.SemiDios) Then
110             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª La cantidad de NPCs disponible para tu rango est· limitada.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
110             Call LogGM(.Name, "Sumoneo " & Declaraciones.SpawnList(NPC).NpcName)
            Else
112             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
140                 Call WriteConsoleMsg(UserIndex, "No hay ningun personaje con ese nick", e_FontTypeNames.FONTTYPE_INFO)
                ...
                End If
            Else
142             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
114                 Call WriteConsoleMsg(UserIndex, "Clan " & UCase$(GuildName) & ": " & modGuilds.m_ListaDeMiembrosOnline(UserIndex, tGuild), e_FontTypeNames.FONTTYPE_GUILDMSG)
                End If
            Else
116             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
104         X = Reader.ReadInt16()
106         Y = Reader.ReadInt16()
...
116         If MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y - 1).ObjInfo.ObjIndex > 0 Then Exit Sub
        
118         If MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y - 1).TileExit.Map <> 0 Then Exit Sub
        
120         If MapData(Mapa).Tile(X, Y).ObjInfo.ObjIndex > 0 Then
122             Call WriteConsoleMsg(UserIndex, "Hay un objeto en el piso en ese lugar", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub

            End If
        
124         If MapData(Mapa).Tile(X, Y).TileExit.Map <> 0 Then
126             Call WriteConsoleMsg(UserIndex, "No podÈs crear un teleport que apunte a la entrada de otro.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
134         With MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y - 1)
...
114         With MapData(Mapa).Tile(X, Y)
...
134                 If MapData(.TileExit.Map).Tile(.TileExit.X, .TileExit.Y).ObjInfo.ObjIndex = 651 Then
...
120             Call SendToAreaByPos(Mapa, X, Y, PrepareMessagePlayWave(waveID, X, Y))
...
106         For Y = .Pos.Y - HALF_Y To .Pos.Y + HALF_Y
108             For X = .Pos.X - HALF_X To .Pos.X + HALF_X

110                 If X > 0 And Y > 0 And X < 101 And Y < 101 Then
                
112                     If MapData(.Pos.Map).Tile(X, Y).ObjInfo.ObjIndex > 0 Then
                    
114                         If ItemNoEsDeMapa(MapData(.Pos.Map).Tile(X, Y).ObjInfo.ObjIndex) Then
                ...
                Else
                    If UserList(tUser).GuildIndex > 0 Then
                        If GuildAlignmentIndex(UserList(tUser).GuildIndex) <> e_ALINEACION_GUILD.ALINEACION_CAOTICA Then
                            Call WriteConsoleMsg(UserIndex, "El miembro no puede ingresar al concilio porque forma parte de un clan que no es caÛtico.", e_FontTypeNames.FONTTYPE_INFO)
                        End If
                    End If
                    
112                 Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(UserName & " fue aceptado en el Consejo de la LegiÛn Oscura.", e_FontTypeNames.FONTTYPE_CONSEJOCAOS))
...
106         For X = 5 To 95
108             For Y = 5 To 95
110                 tObj = MapData(.Pos.Map).Tile(X, Y).ObjInfo.ObjIndex
...
108             If tUser <= 0 Then
                    ...
                    Call SendKickFaction(UserIndex, UserName, 6)
...
138                         Call WriteConsoleMsg(tUser, "Has sido echado del consejo de la LegiÛn Oscura", e_FontTypeNames.FONTTYPE_TALK)
140                        .Faccion.Status = e_Facciones.Caos
                        
142                         Call WarpUserChar(tUser, .Pos.Map, .Pos.X, .Pos.Y)
144                         Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(UserName & " fue expulsado del consejo de la LegiÛn Oscura", e_FontTypeNames.FONTTYPE_CONSEJOCAOS))
...
108             MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).trigger = tTrigger
...
104         tTrigger = MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).trigger
...
126             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If
...
134         Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg(.Name & " baneÛ la IP " & bannedip & " por " & Reason, e_FontTypeNames.FONTTYPE_FIGHT))
...
112             Call WriteConsoleMsg(UserIndex, "Solo podÈs crear hasta " & CStr(MAX_INVENTORY_OBJS) & " unidades", e_FontTypeNames.FONTTYPE_TALK)
                Exit Sub
...
106         If MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).ObjInfo.ObjIndex = 0 Then Exit Sub
...
118             If tUser > 0 Then
                    If UserList(tUser).GuildIndex > 0 Then
                        If GuildAlignmentIndex(UserList(tUser).GuildIndex) = e_ALINEACION_GUILD.ALINEACION_ARMADA Then
                            Call WriteConsoleMsg(UserIndex, "El usuario " & UserName & " deber· abandonar el clan para poder ser echado de las fuerzas del caos.", e_FontTypeNames.FONTTYPE_INFO)
                            Exit Sub
                        End If
                    Else
...
126                     Call WriteConsoleMsg(tUser, .Name & " te ha expulsado en forma definitiva de las fuerzas del caos.", e_FontTypeNames.FONTTYPE_FIGHT)
                    End If
                Else
                    Call SendKickFaction(UserIndex, UserName, 2)
                End If
...
118             If tUser > 0 Then
                    If UserList(tUser).GuildIndex > 0 Then
                        If GuildAlignmentIndex(UserList(tUser).GuildIndex) = e_ALINEACION_GUILD.ALINEACION_ARMADA Then
                            Call WriteConsoleMsg(UserIndex, "El usuario " & UserName & " deber· abandonar el clan para poder ser echado de la armada.", e_FontTypeNames.FONTTYPE_INFO)
                            Exit Sub
                        End If
                    Else
122                     UserList(tUser).Faccion.Reenlistadas = 2
...
126                     Call WriteConsoleMsg(tUser, .Name & " te ha expulsado en forma definitiva de las fuerzas reales.", e_FontTypeNames.FONTTYPE_FIGHT)
                ...
                Else
                    Call SendKickFaction(UserIndex, UserName, 3)
                End If
...
'124                     Call LogGM(.Name, "Borro la pena " & punishment & " de " & UserName & " y la cambiÛ por: " & NewText)
'
...
108         If MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).Blocked = 0 Then
110             MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).Blocked = e_Block.ALL_SIDES Or e_Block.GM
            
            Else
112             MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).Blocked = 0

            End If
        
114         Call Bloquear(True, .Pos.Map, .Pos.X, .Pos.Y, IIf(MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).Blocked > 0, e_Block.ALL_SIDES, 0))
...
108         For Y = .Pos.Y - HALF_Y To .Pos.Y + HALF_Y
110             For X = .Pos.X - HALF_X To .Pos.X + HALF_X

112                 If X > 0 And Y > 0 And X < 101 And Y < 101 Then

114                     If MapData(.Pos.Map).Tile(X, Y).NpcIndex > 0 Then
                    
116                         Call QuitarNPC(MapData(.Pos.Map).Tile(X, Y).NpcIndex)
...
Private Sub HandleLastIP(ByVal UserIndex As Integer)
        ...
        '***************************************************
        'Author: MartÌn Trionfetti (HarThaoS) - Fernando Quinteros (Lord Fers)
...
118                 UserName = Replace(UserName, "+", " ")
                ...
                Call SendGetLastIP(UserIndex, UserName)
            Else
146             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
120                     Call WriteConsoleMsg(UserIndex, "No hay Objeto en slot seleccionado", e_FontTypeNames.FONTTYPE_INFO)
                ...
                Else
122                 Call WriteConsoleMsg(UserIndex, "Slot InvÌlido.", e_FontTypeNames.FONTTYPE_TALK)
...
104         If Oro <= 0 Then Exit Sub
            ...
            'Se asegura que el target es un npc
106         If .flags.TargetNPC = 0 Then
108             Call WriteConsoleMsg(UserIndex, "Primero tenÈs que seleccionar al sacerdote.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
118         If .Faccion.Status = e_Facciones.Ciudadano Or .Faccion.Status = e_Facciones.Armada Or .Faccion.Status = e_Facciones.consejo Or .Faccion.Status = e_Facciones.concilio Or .Faccion.Status = e_Facciones.Caos Or .Stats.ciudadanosMatados = 0 Then
120             Call WriteChatOverHead(UserIndex, "No puedo aceptar tu donaciÛn en este momento...", NpcList(.flags.TargetNPC).Char.charindex, vbWhite)
                Exit Sub
            End If

122         If .GuildIndex <> 0 Then
124             If modGuilds.Alineacion(.GuildIndex) = 1 Then
126                 Call WriteChatOverHead(UserIndex, "Te encuentras en un clan criminal... no puedo aceptar tu donaciÛn.", priest.Char.charindex, vbWhite)
                    Exit Sub
...
130             Call WriteConsoleMsg(UserIndex, "No tienes suficiente dinero.", e_FontTypeNames.FONTTYPE_INFO)
            ...
            Dim Donacion As Long
            If .Stats.ciudadanosMatados > 0 Then
132             Donacion = .Stats.ciudadanosMatados * OroMult * CostoPerdonPorCiudadano
            Else
                Donacion = 10000
            End If
...
144         Call WriteChatOverHead(UserIndex, "°Gracias por tu generosa donaciÛn! Con estas palabras, te libero de todo tipo de pecados. °Que Dios te acompaÒe hijo mÌo!", NpcList(UserList(UserIndex).flags.TargetNPC).Char.charindex, vbYellow)
...
138             Call LogPremios(.Name, UserName, ObjIndex, Cantidad, Motivo)
            Else
140             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo, debes pedir a un Dios que lo de.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
106         Call LogGM(.Name, .Name & " ha cambiado la informaciÛn sobre el BackUp")
        
            'Change the boolean to byte in a fast way
108         If doTheBackUp Then
110             Zona(.ZonaId).Backup = 1
            
            Else
112             Zona(.ZonaId).Backup = 0

            End If
        
            'Change the boolean to string in a fast way
114         Call WriteVar(MapPath & "mapa" & .Pos.Map & ".dat", "Mapa" & .Pos.Map, "backup", Zona(.ZonaId).Backup)
        
116         Call WriteConsoleMsg(UserIndex, "Mapa " & .Pos.Map & " Backup: " & Zona(.ZonaId).Backup, e_FontTypeNames.FONTTYPE_INFO)
...
110         Zona(.ZonaId).Segura = IIf(isMapPk, 1, 0)

112         Call WriteConsoleMsg(UserIndex, "Mapa " & .Pos.Map & " Seguro: " & Zona(.ZonaId).Segura, e_FontTypeNames.FONTTYPE_INFO)
...
106             Select Case UCase$(tStr)
                    ...
                    Case "NEWBIE"
108                     Zona(.ZonaId).Newbie = Not Zona(.ZonaId).Newbie
110                     Call WriteConsoleMsg(UserIndex, "Mapa " & .Pos.Map & ": Newbie = " & Zona(.ZonaId).Newbie, e_FontTypeNames.FONTTYPE_INFO)
112                     Call LogGM(.Name, .Name & " ha cambiado la restricciÛn del mapa " & .Pos.Map & ": Newbie = " & Zona(.ZonaId).Newbie)
                        
114                 Case "SINMAGIA"
116                     Zona(.ZonaId).SinMagia = Not Zona(.ZonaId).SinMagia
118                     Call WriteConsoleMsg(UserIndex, "Mapa " & .Pos.Map & ": SinMagia = " & Zona(.ZonaId).SinMagia, e_FontTypeNames.FONTTYPE_INFO)
120                     Call LogGM(.Name, .Name & " ha cambiado la restricciÛn del mapa " & .Pos.Map & ": SinMagia = " & Zona(.ZonaId).SinMagia)
                          
138                 Case "SININVI"
140                     Zona(.ZonaId).SinInvi = Not Zona(.ZonaId).SinInvi
142                     Call WriteConsoleMsg(UserIndex, "Mapa " & .Pos.Map & ": SinInvi = " & Zona(.ZonaId).SinInvi, e_FontTypeNames.FONTTYPE_INFO)
144                     Call LogGM(.Name, .Name & " ha cambiado la restricciÛn del mapa " & .Pos.Map & ": SinInvi = " & Zona(.ZonaId).SinInvi)
...
110                 Zona(UserList(UserIndex).ZonaId).Terreno = tStr
                
112                 Call WriteVar(MapPath & "mapa" & UserList(UserIndex).Pos.Map & ".dat", "Mapa" & UserList(UserIndex).Pos.Map, "Terreno", tStr)
                
114                 Call WriteConsoleMsg(UserIndex, "Mapa " & .Pos.Map & " Terreno: " & Zona(.ZonaId).Terreno, e_FontTypeNames.FONTTYPE_INFO)
...
118                 Call WriteConsoleMsg(UserIndex, "Igualmente, el Ìnico Ìtil es 'NIEVE' ya que al ingresarlo, la gente muere de frio en el Mapa", e_FontTypeNames.FONTTYPE_INFO)
...
114                 Call WriteConsoleMsg(UserIndex, "Mapa " & .Pos.Map & " Zona: " & Zona(.ZonaId).Terreno, e_FontTypeNames.FONTTYPE_INFO)
                
                Else
            
116                 Call WriteConsoleMsg(UserIndex, "Opciones para terreno: 'BOSQUE', 'NIEVE', 'DESIERTO', 'CIUDAD', 'CAMPO', 'DUNGEON'", e_FontTypeNames.FONTTYPE_INFO)
118                 Call WriteConsoleMsg(UserIndex, "Igualmente, el Ìnico Ìtil es 'DUNGEON' ya que al ingresarlo, NO se sentirÌ el efecto de la lluvia en este mapa.", e_FontTypeNames.FONTTYPE_INFO)
...
106         Call modES.DoBackUp 'Sino lo confunde con la id del paquete
...
Public Sub HandleAlterName(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        Call SendAlterName(UserIndex, UserName, NewName)
...
106             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo. Si el motivo es probar algo ya saben ir a Test", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If
...
116             Call WriteConsoleMsg(UserIndex, "El nivel mÌnimo para participar es " & Torneo.NivelMinimo & ".", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
134             Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg("Se baneÛ la IP  " & bannedip & " del personaje " & UserList(UserIndex).Name & " por bot.", e_FontTypeNames.FONTTYPE_FIGHT))
...
110             Call WriteConsoleMsg(UserIndex, "Primero tenÈs que hacer click sobre el subastador.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub

            End If

112         If NpcList(.flags.TargetNPC).NPCtype <> e_NPCType.Subastador Then
114             Call WriteConsoleMsg(UserIndex, "Primero tenÈs que hacer click sobre el subastador.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
118             Call WriteLocaleMsg(UserIndex, "8", e_FontTypeNames.FONTTYPE_INFO)
                'Call WriteConsoleMsg(UserIndex, "EstÌs demasiado lejos del subastador.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub

            End If
        
120         If .flags.Subastando = False Then
122             Call WriteChatOverHead(UserIndex, "Oye amigo, tu no podÈs decirme cual es la oferta inicial.", NpcList(UserList(UserIndex).flags.TargetNPC).Char.charindex, vbWhite)
                Exit Sub
...
114             Call WriteConsoleMsg(UserIndex, "No podÈs auto ofertar en tus subastas. La proxima vez iras a la carcel...", e_FontTypeNames.FONTTYPE_INFOIAO)
...
136                 Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Oferta mejorada por: " & .Name & " (Ofrece " & PonerPuntos(Oferta) & " monedas de oro) - Tiempo Extendido. Escribe /SUBASTA para mas informaciÛn.", e_FontTypeNames.FONTTYPE_SUBASTA))
...
140                 Subasta.TiempoRestanteSubasta = Subasta.TiempoRestanteSubasta + 30
                Else
142                 Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Oferta mejorada por: " & .Name & " (Ofrece " & PonerPuntos(Oferta) & " monedas de oro). Escribe /SUBASTA para mas informaciÛn.", e_FontTypeNames.FONTTYPE_SUBASTA))
...
130                     Call modSendData.SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("[" & .Name & "] " & chat, e_FontTypeNames.FONTTYPE_GLOBAL))
                        ...
                        'TODO : Con la 0.12.1 se debe definir si esto vuelve o se borra (/CMSG overhead)
                        'Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.map, "||" & vbBlue & "Ì< " & rData & " >Ì" & CStr(UserList(UserIndex).Char.CharIndex))
                    End If
...
108             Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor ¬ª Chat general habilitado. Escribe" & Chr(34) & "/CONSOLA" & Chr(34) & " o " & Chr(34) & ";" & Chr(34) & " y su mensaje para utilizarlo.", e_FontTypeNames.FONTTYPE_SERVER))
110             EstadoGlobal = True
            Else
112             Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor ¬ª Chat General deshabilitado.", e_FontTypeNames.FONTTYPE_SERVER))
...
150                 Call FindLegalPos(tempIndex, UserList(tempIndex).Pos.Map, UserList(tempIndex).Pos.X, UserList(tempIndex).Pos.Y)
152                 Call WarpUserChar(tempIndex, nPos.Map, nPos.X, nPos.Y, True)
                    
116                 Call WriteConsoleMsg(UserIndex, "Servidor ¬ª AcciÛn realizada con exito! La nueva posicion de " & UserName & " es: " & UserList(tempIndex).Pos.Map & "-" & UserList(tempIndex).Pos.X & "-" & UserList(tempIndex).Pos.Y & ".", e_FontTypeNames.FONTTYPE_INFO)
                    'HarTaoS ReyarB ver porque si el usuario esta online lo dice igual
                Else
118                 Call WriteConsoleMsg(UserIndex, "Servidor ¬ª El usuario debe estar deslogueado para dicha solicitud!", e_FontTypeNames.FONTTYPE_INFO)
...
110             Call CancelarSolicitudReto(.flags.AceptoReto, .Name & " ha cancelado su admisiÛn.")
...
112             Call WriteLocaleMsg(UserIndex, "77", e_FontTypeNames.FONTTYPE_INFO)
            ...
            'Validate target NPC
114         If .flags.TargetNPC = 0 Then
116             Call WriteConsoleMsg(UserIndex, "Primero tenÈs que seleccionar un personaje, haz click izquierdo sobre Èl.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
130         If Not EsGM(UserIndex) Then
                ...
                Call SendTransferGold(UserIndex, UserName, Cantidad)
                
                'Guardamos el evento
                Call SendCharacterEvent(UserIndex, e_EventType.TransferGold, "Transfiere " & Cantidad & " a " & UserName, 0, Cantidad, Cantidad)
...
104         SlotNuevo = Reader.ReadInt8()
            ...
            'HarThaoS: Si es un hilo de pesca y lo estoy arrastrando en una caÒa rota borro del slot viejo y en el nuevo pongo la caÒa correspondiente
            ...
            If .Invent.Object(SlotViejo).ObjIndex = 2183 Then
                ...
                End Select
                ObjCania.Amount = 1
                'si el objeto que estaba pisando era una caÒa rota.
...
132                     If .Invent.DaÒoMagicoEqpSlot = SlotViejo Then
134                         .Invent.DaÒoMagicoEqpSlot = SlotNuevo
...
158                         .Invent.EscudoEqpSlot = SlotNuevo
                        ...
                        'Cambiamos si alguno es municiÛn
...
212                         .Invent.Object(SlotViejo).Equipped = 0
                    ...
                    'Cambiamos si alguno es un anillo
214                 If .Invent.DaÒoMagicoEqpSlot = SlotViejo Then
216                     .Invent.DaÒoMagicoEqpSlot = SlotNuevo
218                 ElseIf .Invent.DaÒoMagicoEqpSlot = SlotNuevo Then
220                     .Invent.DaÒoMagicoEqpSlot = SlotViejo
...
106             Call WriteConsoleMsg(UserIndex, "Ya perteneces a un clan, no podÈs fundar otro.", e_FontTypeNames.FONTTYPE_INFOIAO)
                Exit Sub
            End If

108         If UserList(UserIndex).Stats.ELV < 35 Then
110             Call WriteConsoleMsg(UserIndex, "Para fundar un clan debes ser nivel 35 y tener en tu inventario las 2 gemas: Gema Polar(1), Gema Roja(1).", e_FontTypeNames.FONTTYPE_INFOIAO)
                Exit Sub
            End If
...
116         Call WriteConsoleMsg(UserIndex, "Servidor ¬ª °Comenzamos a fundar el clan! Ingresa todos los datos solicitados.", e_FontTypeNames.FONTTYPE_INFOIAO)
...
108                 Call SendData(SendTarget.ToGuildMembers, .GuildIndex, PrepareMessageConsoleMsg("Clan> [" & .Name & "] solicita apoyo de su clan en " & get_map_name(.Pos.Map) & " (" & .Pos.Map & "-" & .Pos.X & "-" & .Pos.Y & "). Puedes ver su ubicaciÛn en el mapa del mundo.", e_FontTypeNames.FONTTYPE_GUILD))
...
112                 Call SendData(SendTarget.ToGuildMembers, .GuildIndex, PrepareMessageUbicacionLlamada(.Pos.Map, .Pos.X, .Pos.Y))
                ...
                Else
114                 Call WriteConsoleMsg(UserIndex, "Servidor ¬ª El nivel de tu clan debe ser 2 para utilizar esta opciÛn.", e_FontTypeNames.FONTTYPE_INFOIAO)
...
118                         Call WriteConsoleMsg(UserIndex, "No podÈs casarte contigo mismo.", e_FontTypeNames.FONTTYPE_INFO)
...
136         X = Reader.ReadInt16
138         Y = Reader.ReadInt16
...
114                             Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Eventos> Todavia nadie fue capaz de encontar el tesoro, recorda que se encuentra en " & get_map_name(TesoroNumMapa) & "(" & TesoroNumMapa & "). øQuien sera el valiente que lo encuentre?", e_FontTypeNames.FONTTYPE_TALK))
...
150                             Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Eventos> TodavÌa nadie logrÛ matar el NPC que se encuentra en el mapa " & NpcList(npc_index_evento).Pos.Map & ".", e_FontTypeNames.FONTTYPE_TALK))
...
154                             Call WriteConsoleMsg(UserIndex, "Ya hay una busqueda del tesoro activa.", e_FontTypeNames.FONTTYPE_INFO)
                ...
                End Select
            Else
156             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª No estas habilitado para hacer Eventos.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
108                 Call CompletarAccionFin(UserIndex)
                Else
110                 Call WriteConsoleMsg(UserIndex, "Servidor ¬ª La acciÛn que solicitas no se corresponde.", e_FontTypeNames.FONTTYPE_SERVER)

                End If

            Else
112             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Tu no tenias ninguna acciÛn pendiente. ", e_FontTypeNames.FONTTYPE_SERVER)
...
108                 Call WriteWorkRequestTarget(UserIndex, e_Skill.Grupo)
                Else
110                 Call WriteConsoleMsg(UserIndex, "°No podÈs invitar a m·s personas!", e_FontTypeNames.FONTTYPE_INFO)
...
112             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª El nivel de tu clan debe ser 3 para utilizar esta opciÛn.", e_FontTypeNames.FONTTYPE_INFOIAO)
                Exit Sub
            End If
...
128                                     Call WriteLocaleMsg(UserList(UserIndex).Grupo.PropuestaDe, "36", e_FontTypeNames.FONTTYPE_INFOIAO)
                                        'Call WriteConsoleMsg(UserList(UserIndex).Grupo.PropuestaDe, "ÌEl grupo a sido creado!", e_FontTypeNames.FONTTYPE_INFOIAO)
...
150                                 Log = "Repuesta Afirmativa 1-6 "
                                    'Call WriteConsoleMsg(UserList(UserIndex).Grupo.PropuestaDe, "Ì" & UserList(UserIndex).name & " a sido aÒadido al grupo!", e_FontTypeNames.FONTTYPE_INFOIAO)
152                                 Call WriteLocaleMsg(UserList(UserIndex).Grupo.PropuestaDe, "40", e_FontTypeNames.FONTTYPE_INFOIAO, UserList(UserIndex).Name)
                                
154                                 Call WriteConsoleMsg(UserIndex, "°Has sido aÒadido al grupo!", e_FontTypeNames.FONTTYPE_INFOIAO)
...
166                         Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Solicitud de grupo invalida, reintente...", e_FontTypeNames.FONTTYPE_SERVER)
...
232                         Call IniciarComercioConUsuario(UserIndex, UserList(UserIndex).flags.TargetUser)
                        ...
                        Else
234                         Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Solicitud de comercio invalida, reintente...", e_FontTypeNames.FONTTYPE_SERVER)
...
118                 Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg("Servidor ¬ª " & .Name & " ha desbaneado la cuenta de " & UserNameOEmail & ".", e_FontTypeNames.FONTTYPE_SERVER))
                Else
...
122                 Call WriteConsoleMsg(UserIndex, "Ya hay un evento en curso. Finalicelo con /FINEVENTO primero.", e_FontTypeNames.FONTTYPE_New_Eventos)
                ...
                End If
            Else
124             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Solo Administradores pueder crear estos eventos.", e_FontTypeNames.FONTTYPE_INFO)
            End If
...
'108         If (.flags.Privilegios And (e_PlayerType.Admin Or e_PlayerType.Dios Or e_PlayerType.SemiDios)) Then
'110             Call modAdmin.BanTemporal(UserName, dias, Reason, UserList(UserIndex).Name)
'            Else
'112             Call WriteConsoleMsg(UserIndex, "Servidor ¬ª Comando deshabilitado para tu cargo.", e_FontTypeNames.FONTTYPE_INFO)
'            End If
'
...
154                     Call WriteConsoleMsg(UserIndex, "Has viajado por varios dÌas, te sientes exhausto!", e_FontTypeNames.FONTTYPE_WARNING)
...
184                 Call WriteConsoleMsg(UserIndex, "Has viajado por varios dÌas, te sientes exhausto!", e_FontTypeNames.FONTTYPE_WARNING)
...
110         Call SendData(SendTarget.ToIndex, UserIndex, PrepareMessageChatOverHead("No tengo ninguna misiÛn para ti.", NpcList(NpcIndex).Char.charindex, vbWhite))
            Exit Sub
...
120                 Call WriteChatOverHead(UserIndex, "Debes completar la quest " & QuestList(QuestList(NpcList(NpcIndex).QuestNumber(Indice)).RequiredQuest).nombre & " para emprender esta misiÛn.", NpcList(NpcIndex).Char.charindex, vbYellow)
                    Exit Sub
...
124             Call WriteChatOverHead(UserIndex, "Debes ser por lo menos nivel " & QuestList(NpcList(NpcIndex).QuestNumber(Indice)).RequiredLevel & " para emprender esta misiÛn.", NpcList(NpcIndex).Char.charindex, vbYellow)
                Exit Sub
...
144             Call WriteConsoleMsg(UserIndex, "Has aceptado la misiÛn " & Chr(34) & QuestList(.QuestIndex).nombre & Chr(34) & ".", e_FontTypeNames.FONTTYPE_INFOIAO)
146
...
104             Response = "[Info de la zona " & .ZonaId & " en " & .Pos.Map & "," & .Pos.X & "," & .Pos.Y & "]" & vbNewLine
106             Response = Response & "Nombre = " & Zona(.ZonaId).Zona_name & vbNewLine
108             Response = Response & "Seguro = " & Zona(.ZonaId).Segura & vbNewLine
110             Response = Response & "Newbie = " & Zona(.ZonaId).Newbie & vbNewLine
112             Response = Response & "Nivel = " & Zona(.ZonaId).MinLevel & "/" & Zona(.ZonaId).MaxLevel & vbNewLine
114             Response = Response & "SinInvi = " & Zona(.ZonaId).SinInvi & vbNewLine
116             Response = Response & "SinMagia = " & Zona(.ZonaId).SinMagia & vbNewLine
118             Response = Response & "SoloClanes = " & Zona(.ZonaId).SoloClanes & vbNewLine
120             Response = Response & "Faccion = " & Zona(.ZonaId).Faccion & vbNewLine
124             Response = Response & "Salida = " & Zona(.ZonaId).Salida.Map & "-" & Zona(.ZonaId).Salida.X & "-" & Zona(.ZonaId).Salida.Y & vbNewLine
126             Response = Response & "Terreno = " & Zona(.ZonaId).Terreno & vbNewLine
...
108             Call WriteConsoleMsg(UserIndex, "No podÈs denunciar a un administrador.", e_FontTypeNames.FONTTYPE_INFO)
                Exit Sub
...
116         Denuncia = "[√öltimos mensajes de " & UserList(tUser).Name & "]" & vbNewLine
...
102         chatMessage = "[" & UserList(UserIndex).Name & "] " & Reader.ReadString8
            ...
            'El mensaje se lo envÌo al destino
...
102                 mensaje = "AntiCheat> El usuario " & UserList(UserIndex).Name & " est· utilizando macro de COORDENADAS."
                ...
                Case tMacro.borrarCartel
                    mensaje = "AntiCheat> El usuario " & UserList(UserIndex).Name & " est· utilizando macro de CARTELEO."
                    Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg(mensaje, e_FontTypeNames.FONTTYPE_INFO))
            End Select
...
114                 Call LogGM(.Name, "ForzÛ el evento InvasiÛn en Banderbille.")
                
116             Case "INVASION CARCEL"
118                 Call IniciarEvento(TipoEvento.Invasion, 2)
120                 Call LogGM(.Name, "ForzÛ el evento InvasiÛn en Carcel.")
...
108         If Zona(.ZonaId).Terreno = "NEWBIE" Or MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).trigger = CARCEL Then
...
114             Call WriteConsoleMsg(UserIndex, "No puedes usar este comando en prisiÛn.", e_FontTypeNames.FONTTYPE_FIGHT)
                Exit Sub

            End If
            
116         If .flags.EnReto Then
118             Call WriteConsoleMsg(UserIndex, "No podÈs regresar desde un reto. Usa /ABANDONAR para admitir la derrota y volver a la ciudad.", e_FontTypeNames.FONTTYPE_FIGHT)
                Exit Sub
            End If

120         If .flags.Traveling = 0 Then
            
122             If .ZonaId <> MapData(Ciudades(.Hogar).MapaResu).Tile(Ciudades(.Hogar).Resux, Ciudades(.Hogar).Resuy).ZonaId Then
...
Private Sub HandleRomperCania(ByVal UserIndex As Integer)
    ...
    For LoopC = 1 To MAX_INVENTORY_SLOTS
        ...
        'Rastreo la caÒa que est· usando en el inventario y se la rompo
...
Private Sub HandleRepeatMacro(ByVal UserIndex As Integer)
    ...
    'Call LogMacroCliente("El usuario " & UserList(UserIndex).name & " iterÛ el paquete click o u." & GetTickCount)
    Exit Sub
...
Private Sub HandleBuyShopItem(ByVal UserIndex As Integer)
    ...
    Call ModShopAO.init_transaction(obj_to_buy, UserIndex)
...
Private Sub HandlePublicarPersonajeMAO(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        '    Call WriteConsoleMsg(UserIndex, "El personaje ya est· publicado.", e_FontTypeNames.FONTTYPE_INFOBOLD)
        '    Exit Sub
        'End If
        '
...
Private Sub HandleDeleteItem(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If Zona(UserList(UserIndex).ZonaId).Segura = 0 Then
