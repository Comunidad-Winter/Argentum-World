...
Begin VB.Form frmMain 
   ...
   Caption         =   "Servidor Argentum World"
   ClientHeight    =   6255
   ClientLeft      =   1950
   ClientTop       =   1695
   ClientWidth     =   8595
...
102     Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor Â» Cerrando servidor y lanzando nuevo parche.", e_FontTypeNames.FONTTYPE_PROMEDIO_MENOR))
...
100     Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor Â» " & BroadMsg.Text, e_FontTypeNames.FONTTYPE_SERVER))
...
102     Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor Â» Cerrando servidor.", e_FontTypeNames.FONTTYPE_PROMEDIO_MENOR))
...
100     If MsgBox("¡Atención! Si cierra el servidor puede provocar la perdida de datos. ¿Desea hacerlo de todas maneras?", vbYesNo) = vbYes Then
...
Private Sub TimerMeteorologia_Timer()
    ...
    If TimerMeteorologico = 7 Then
        ...
        If ProbabilidadNublar = 1 Then
            ...
            Call AgregarAConsola("Servidor Â» Empezaron las nubes")
        ...
        Else
            ...
            Call AgregarAConsola("Servidor Â» Tranquilo, no hay nubes ni va a llover.")
            Call ResetMeteo
            Exit Sub
    ...
    If TimerMeteorologico < 7 And TimerMeteorologico > 3 Then
        ...
        Call AgregarAConsola("Servidor Â» Truenos y nubes activados.")
        Exit Sub
    ...
    If TimerMeteorologico = 3 Then
        ...
        If ProbabilidadLLuvia = 1 Then
            ...
            Call AgregarAConsola("Servidor Â» Lloviendo.")
        ...
        Else
            ...
            Call SendData(SendTarget.ToAll, 0, PrepareMessageNieblandoToggle(IntensidadDeNubes))
            Call AgregarAConsola("Servidor Â» Truenos y nubes desactivados.")
...
Private Sub tmrDBCheck_Timer()
If Not DBIsConnected Then
...
Private Sub tPiqueteC_Timer()
    ...
    For i = 1 To LastUser
        ...
        If UserList(i).flags.UserLogged Then
            If MapData(UserList(i).Pos.Map).Tile(UserList(i).Pos.X, UserList(i).Pos.Y).trigger = e_Trigger.ANTIPIQUETE Then
                UserList(i).Counters.PiqueteC = UserList(i).Counters.PiqueteC + 1
                'Call WriteConsoleMsg(i, "EstÃ¯¿Â½s obstruyendo la via pÃ¯¿Â½blica, muÃ¯¿Â½vete o serÃ¯¿Â½s encarcelado!", e_FontTypeNames.FONTTYPE_INFO)
                ...
                If UserList(i).Counters.PiqueteC > 10 Then
                    ...
                    'Ojo! No sÃ¯¿Â½ si se puede abusar de esto para evitar los 10 segundos al salir
