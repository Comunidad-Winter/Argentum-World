...
124                 If Zona(UserList(UserIndex).ZonaId).SoloClanes And Zona(UserList(UserIndex).ZonaId).Salida.Map <> 0 Then
126                     Call WriteConsoleMsg(UserIndex, "Necesitas un clan para pertenecer en este mapa.", e_FontTypeNames.FONTTYPE_INFO)
128                     Call WarpUserChar(UserIndex, Zona(UserList(UserIndex).ZonaId).Salida.Map, Zona(UserList(UserIndex).ZonaId).Salida.X, Zona(UserList(UserIndex).ZonaId).Y, True)
                    Else
...
106     If UserList(UserIndex).Stats.ELV < 35 Then
108         refError = "Para fundar un clan debes ser nivel 35 y tener en tu inventario las Gemas Polar y Roja (Fundación)."
            Exit Function
        End If
    
110     If Not TieneObjetos(407, 1, UserIndex) Then
112         refError = "Para fundar un clan debes ser nivel 35 y tener en tu inventario las Gemas Polar y Roja (Fundación)."
            Exit Function
        End If
    
114     If Not TieneObjetos(408, 1, UserIndex) Then
116         refError = "Para fundar un clan debes ser nivel 35 y tener en tu inventario las Gemas Polar y Roja (Fundación)."
            Exit Function
        End If
...
100     Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor Â» Revisando elecciones", e_FontTypeNames.FONTTYPE_SERVER))
...
108                 Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor Â» " & guilds(i).GetLeader & " es el nuevo líder de " & guilds(i).GuildName & "!", e_FontTypeNames.FONTTYPE_SERVER))
...
128     Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor Â» Elecciones revisadas", e_FontTypeNames.FONTTYPE_SERVER))
        Exit Sub
errh:
...
146     Call SendData(SendTarget.ToDiosesYclan, NuevoGuildIndex, PrepareMessageGuildChat("Clan: [" & UserList(UserIndex).Name & "] ha enviado solicitud para unirse al clan.", 21))
