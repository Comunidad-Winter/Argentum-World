...
148         Daño = DañoBase - Defensa
            Call DañoExtraPorCombo(AtacanteIndex, VictimaIndex, Daño, tipoDanio.t_golpe)
...
172                     Call WriteLocaleMsg(AtacanteIndex, "383", e_FontTypeNames.FONTTYPE_FIGHT, .Name & "Â¬" & DañoStr)
                    End If
                    ' Y a la víctima
174                 If .ChatCombate = 1 Then
176                     Call WriteLocaleMsg(VictimaIndex, "385", e_FontTypeNames.FONTTYPE_FIGHT, UserList(AtacanteIndex).Name & "Â¬" & DañoStr)
                    End If
...
184             If RandomNumber(1, 100) <= ProbabilidadApunalar(AtacanteIndex) Then
                    ' Daño del apuñalamiento
186                 DañoExtra = Daño * ModicadorApunalarClase(UserList(AtacanteIndex).clase)

188                 DañoStr = PonerPuntos(DañoExtra)
                
                    ' Mostramos en consola el golpe al atacante solo si tiene activado el chat de combate
190                 If UserList(AtacanteIndex).ChatCombate = 1 Then
192                     Call WriteLocaleMsg(AtacanteIndex, "210", e_FontTypeNames.FONTTYPE_INFOBOLD, .Name & "Â¬" & DañoStr)
                    End If
                    ' Mostramos en consola el golpe a la victima independientemente de la configuración de chat
196                 Call WriteLocaleMsg(VictimaIndex, "211", e_FontTypeNames.FONTTYPE_INFOBOLD, UserList(AtacanteIndex).Name & "Â¬" & DañoStr)
...
230                 Call WriteLocaleMsg(VictimaIndex, "387", e_FontTypeNames.FONTTYPE_FIGHT, UserList(AtacanteIndex).Name & "Â¬" & DañoStr)
                End If
...
104             PuedeApunalar = (.clase = e_Class.Assasin Or .Stats.UserSkills(e_Skill.Apunalar) >= MIN_APUÃ‘ALAR) And ObjData(.Invent.WeaponEqpObjIndex).Apuñala = 1
            End If
