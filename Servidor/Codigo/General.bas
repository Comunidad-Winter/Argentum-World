...
102     If toMap Then
104         Call SendToAreaByPos(sndIndex, X, Y, PrepareMessage_BlockPosition(X, Y, b))
        Else
...
106     ModPuerta = ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex).Subtipo

108     Select Case ModPuerta
        
            Case 0
                ' Bloqueos superiores
110             Call Bloquear(toMap, sndIndex, X, Y, MapData(Map).Tile(X, Y).Blocked)
112             Call Bloquear(toMap, sndIndex, X - 1, Y, MapData(Map).Tile(X - 1, Y).Blocked)

                ' Bloqueos inferiores
114             Call Bloquear(toMap, sndIndex, X, Y + 1, MapData(Map).Tile(X, Y + 1).Blocked)
116             Call Bloquear(toMap, sndIndex, X - 1, Y + 1, MapData(Map).Tile(X - 1, Y + 1).Blocked)
...
120         Case 2
                ' Bloqueos superiores
122             Call Bloquear(toMap, sndIndex, X, Y - 1, MapData(Map).Tile(X, Y - 1).Blocked)
124             Call Bloquear(toMap, sndIndex, X - 1, Y - 1, MapData(Map).Tile(X - 1, Y - 1).Blocked)
126             Call Bloquear(toMap, sndIndex, X + 1, Y - 1, MapData(Map).Tile(X + 1, Y - 1).Blocked)
                ' Bloqueos inferiores
128             Call Bloquear(toMap, sndIndex, X, Y, MapData(Map).Tile(X, Y).Blocked)
130             Call Bloquear(toMap, sndIndex, X - 1, Y, MapData(Map).Tile(X - 1, Y).Blocked)
132             Call Bloquear(toMap, sndIndex, X + 1, Y, MapData(Map).Tile(X + 1, Y).Blocked)
                
134         Case 3
                ' Bloqueos superiores
136             Call Bloquear(toMap, sndIndex, X, Y, MapData(Map).Tile(X, Y).Blocked)
138             Call Bloquear(toMap, sndIndex, X - 1, Y, MapData(Map).Tile(X - 1, Y).Blocked)
140             Call Bloquear(toMap, sndIndex, X + 1, Y, MapData(Map).Tile(X + 1, Y).Blocked)
                ' Bloqueos inferiores
142             Call Bloquear(toMap, sndIndex, X, Y + 1, MapData(Map).Tile(X, Y + 1).Blocked)
144             Call Bloquear(toMap, sndIndex, X - 1, Y + 1, MapData(Map).Tile(X - 1, Y + 1).Blocked)
146             Call Bloquear(toMap, sndIndex, X + 1, Y + 1, MapData(Map).Tile(X + 1, Y + 1).Blocked)

148         Case 4
                ' Bloqueos superiores
150             Call Bloquear(toMap, sndIndex, X, Y, MapData(Map).Tile(X, Y).Blocked)
                ' Bloqueos inferiores
152             Call Bloquear(toMap, sndIndex, X, Y + 1, MapData(Map).Tile(X, Y + 1).Blocked)

154         Case 5 'Ver WyroX
                ' Bloqueos vertical ver ReyarB
156             Call Bloquear(toMap, sndIndex, X + 1, Y, MapData(Map).Tile(X + 1, Y).Blocked)
158             Call Bloquear(toMap, sndIndex, X + 1, Y - 1, MapData(Map).Tile(X + 1, Y - 1).Blocked)

                ' Bloqueos horizontal
160             Call Bloquear(toMap, sndIndex, X, Y - 2, MapData(Map).Tile(X, Y - 2).Blocked)
162             Call Bloquear(toMap, sndIndex, X - 1, Y - 2, MapData(Map).Tile(X - 1, Y - 2).Blocked)


164         Case 6 ' Ver WyroX
                ' Bloqueos superiores ver ReyarB
166             Call Bloquear(toMap, sndIndex, X, Y, MapData(Map).Tile(X, Y).Blocked)
168             Call Bloquear(toMap, sndIndex, X, Y - 1, MapData(Map).Tile(X, Y - 1).Blocked)

                ' Bloqueos inferiores
170             Call Bloquear(toMap, sndIndex, X, Y - 2, MapData(Map).Tile(X, Y - 2).Blocked)
172             Call Bloquear(toMap, sndIndex, X + 1, Y - 2, MapData(Map).Tile(X + 1, Y - 2).Blocked)
...
Sub BloquearPuerta(ByVal Map As Integer, _
        ...
        'ver reyarb
100     ModPuerta = ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex).Subtipo
...
102     Select Case ModPuerta
            ...
            Case 0 'puerta 2 tiles
                ...
                ' Bloqueos superiores
104             MapData(Map).Tile(X, Y).Blocked = IIf(Bloquear, MapData(Map).Tile(X, Y).Blocked Or e_Block.NORTH, MapData(Map).Tile(X, Y).Blocked And Not e_Block.NORTH)
106             MapData(Map).Tile(X - 1, Y).Blocked = IIf(Bloquear, MapData(Map).Tile(X - 1, Y).Blocked Or e_Block.NORTH, MapData(Map).Tile(X - 1, Y).Blocked And Not e_Block.NORTH)

                ' Cambio bloqueos inferiores
108             MapData(Map).Tile(X, Y + 1).Blocked = IIf(Bloquear, MapData(Map).Tile(X, Y + 1).Blocked Or e_Block.SOUTH, MapData(Map).Tile(X, Y + 1).Blocked And Not e_Block.SOUTH)
110             MapData(Map).Tile(X - 1, Y + 1).Blocked = IIf(Bloquear, MapData(Map).Tile(X - 1, Y + 1).Blocked Or e_Block.SOUTH, MapData(Map).Tile(X - 1, Y + 1).Blocked And Not e_Block.SOUTH)
...
114         Case 2 ' puerta 3 tiles 1 arriba
                ' Bloqueos superiores
116             MapData(Map).Tile(X, Y - 1).Blocked = IIf(Bloquear, MapData(Map).Tile(X, Y - 1).Blocked Or e_Block.NORTH, MapData(Map).Tile(X, Y - 1).Blocked And Not e_Block.NORTH)
118             MapData(Map).Tile(X - 1, Y - 1).Blocked = IIf(Bloquear, MapData(Map).Tile(X - 1, Y - 1).Blocked Or e_Block.NORTH, MapData(Map).Tile(X - 1, Y - 1).Blocked And Not e_Block.NORTH)
120             MapData(Map).Tile(X + 1, Y - 1).Blocked = IIf(Bloquear, MapData(Map).Tile(X + 1, Y - 1).Blocked Or e_Block.NORTH, MapData(Map).Tile(X + 1, Y - 1).Blocked And Not e_Block.NORTH)
                ' Cambio bloqueos inferiores
122             MapData(Map).Tile(X, Y).Blocked = IIf(Bloquear, MapData(Map).Tile(X, Y).Blocked Or e_Block.SOUTH, MapData(Map).Tile(X, Y).Blocked And Not e_Block.SOUTH)
124             MapData(Map).Tile(X - 1, Y).Blocked = IIf(Bloquear, MapData(Map).Tile(X - 1, Y).Blocked Or e_Block.SOUTH, MapData(Map).Tile(X - 1, Y).Blocked And Not e_Block.SOUTH)
126             MapData(Map).Tile(X + 1, Y).Blocked = IIf(Bloquear, MapData(Map).Tile(X + 1, Y).Blocked Or e_Block.SOUTH, MapData(Map).Tile(X + 1, Y).Blocked And Not e_Block.SOUTH)
                
128         Case 3 ' puerta 3 tiles
                ' Bloqueos superiores
130             MapData(Map).Tile(X, Y).Blocked = IIf(Bloquear, MapData(Map).Tile(X, Y).Blocked Or e_Block.NORTH, MapData(Map).Tile(X, Y).Blocked And Not e_Block.NORTH)
132             MapData(Map).Tile(X - 1, Y).Blocked = IIf(Bloquear, MapData(Map).Tile(X - 1, Y).Blocked Or e_Block.NORTH, MapData(Map).Tile(X - 1, Y).Blocked And Not e_Block.NORTH)
134             MapData(Map).Tile(X + 1, Y).Blocked = IIf(Bloquear, MapData(Map).Tile(X + 1, Y).Blocked Or e_Block.NORTH, MapData(Map).Tile(X + 1, Y).Blocked And Not e_Block.NORTH)
                ' Cambio bloqueos inferiores
136             MapData(Map).Tile(X, Y + 1).Blocked = IIf(Bloquear, MapData(Map).Tile(X, Y + 1).Blocked Or e_Block.SOUTH, MapData(Map).Tile(X, Y + 1).Blocked And Not e_Block.SOUTH)
138             MapData(Map).Tile(X - 1, Y + 1).Blocked = IIf(Bloquear, MapData(Map).Tile(X - 1, Y + 1).Blocked Or e_Block.SOUTH, MapData(Map).Tile(X - 1, Y + 1).Blocked And Not e_Block.SOUTH)
140             MapData(Map).Tile(X + 1, Y + 1).Blocked = IIf(Bloquear, MapData(Map).Tile(X + 1, Y + 1).Blocked Or e_Block.SOUTH, MapData(Map).Tile(X + 1, Y + 1).Blocked And Not e_Block.SOUTH)
        
142         Case 4 'puerta 1 tiles
                ' Bloqueos superiores
144             MapData(Map).Tile(X, Y).Blocked = IIf(Bloquear, MapData(Map).Tile(X, Y).Blocked Or e_Block.NORTH, MapData(Map).Tile(X, Y).Blocked And Not e_Block.NORTH)
                ' Cambio bloqueos inferiores
146             MapData(Map).Tile(X, Y + 1).Blocked = IIf(Bloquear, MapData(Map).Tile(X, Y + 1).Blocked Or e_Block.SOUTH, MapData(Map).Tile(X, Y + 1).Blocked And Not e_Block.SOUTH)
                
148         Case 5 'Ver WyroX
                ' Bloqueos  vertical ver ReyarB
150             MapData(Map).Tile(X + 1, Y).Blocked = IIf(Bloquear, MapData(Map).Tile(X + 1, Y).Blocked Or e_Block.ALL_SIDES, MapData(Map).Tile(X + 1, Y).Blocked And Not e_Block.ALL_SIDES)
152             MapData(Map).Tile(X + 1, Y - 1).Blocked = IIf(Bloquear, MapData(Map).Tile(X + 1, Y - 1).Blocked Or e_Block.ALL_SIDES, MapData(Map).Tile(X + 1, Y - 1).Blocked And Not e_Block.ALL_SIDES)
                
                ' Cambio horizontal
154             MapData(Map).Tile(X, Y - 2).Blocked = IIf(Bloquear, MapData(Map).Tile(X, Y - 2).Blocked Or e_Block.ALL_SIDES, MapData(Map).Tile(X, Y - 2).Blocked And Not e_Block.ALL_SIDES)
156             MapData(Map).Tile(X - 1, Y - 2).Blocked = IIf(Bloquear, MapData(Map).Tile(X - 1, Y - 2).Blocked Or e_Block.ALL_SIDES, MapData(Map).Tile(X - 1, Y - 2).Blocked And Not e_Block.ALL_SIDES)


158         Case 6 ' Ver Wyrox
                ' Bloqueos vertical ver ReyarB
160             MapData(Map).Tile(X - 1, Y).Blocked = IIf(Bloquear, MapData(Map).Tile(X - 1, Y).Blocked Or e_Block.ALL_SIDES, MapData(Map).Tile(X - 1, Y).Blocked And Not e_Block.ALL_SIDES)
162             MapData(Map).Tile(X - 1, Y - 1).Blocked = IIf(Bloquear, MapData(Map).Tile(X - 1, Y - 1).Blocked Or e_Block.ALL_SIDES, MapData(Map).Tile(X - 1, Y - 1).Blocked And Not e_Block.ALL_SIDES)
                
                ' Cambio bloqueos Puerta abierta
164             MapData(Map).Tile(X, Y - 2).Blocked = IIf(Bloquear, MapData(Map).Tile(X, Y - 2).Blocked Or e_Block.ALL_SIDES, MapData(Map).Tile(X, Y - 2).Blocked And Not e_Block.ALL_SIDES)
166             MapData(Map).Tile(X + 1, Y + 2).Blocked = IIf(Bloquear, MapData(Map).Tile(X + 1, Y - 2).Blocked Or e_Block.ALL_SIDES, MapData(Map).Tile(X + 1, Y - 2).Blocked And Not e_Block.ALL_SIDES)
...
102         If ((MapData(Map).Tile(X, Y).Graphic(1) >= 22552 And MapData(Map).Tile(X, Y).Graphic(1) <= 22599) Or (MapData(Map).Tile(X, Y).Graphic(1) >= 7283 And MapData(Map).Tile(X, Y).Graphic(1) <= 7378) Or (MapData(Map).Tile(X, Y).Graphic(1) >= 13387 And MapData(Map).Tile(X, Y).Graphic(1) <= 13482)) And MapData(Map).Tile(X, Y).Graphic(2) = 0 Then
104             HayCosta = True
            Else
106             HayCosta = False
...
Private Function HayLava(ByVal Map As Integer, ByVal X As Integer, ByVal Y As Integer) As Boolean
        ...
        '***************************************************
100     If Map > 0 And Map < NumMaps + 1 And X > 0 And X < 101 And Y > 0 And Y < 101 Then
102         If MapData(Map).Tile(X, Y).Graphic(1) >= 5837 And MapData(Map).Tile(X, Y).Graphic(1) <= 5852 Then
104             HayLava = True
            Else
106             HayLava = False
...
106         For Y = 1 To Mapinfo(MapaActual).Height
108             For X = 1 To Mapinfo(MapaActual).Width

110
                
112                 If MapData(MapaActual).Tile(X, Y).ObjInfo.ObjIndex = FOGATA Then
                        If Zona(MapData(MapaActual).Tile(X, Y).ZonaId).Lluvia Then
...
118     ListaClases(e_Class.Cleric) = "Clerigo"
...
128     ListaClases(e_Class.Paladin) = "Paladin"
130     ListaClases(e_Class.Hunter) = "Cazador"
134     ListaClases(e_Class.Pirat) = "Pirata"
136     ListaClases(e_Class.Thief) = "Ladron"
...
150     SkillsNames(e_Skill.Apunalar) = "Apunalar"
...
170     SkillsNames(e_Skill.Resistencia) = "Resistencia magica"
172     SkillsNames(e_Skill.Talar) = "Tala"
174     SkillsNames(e_Skill.Pescar) = "Pesca"
176     SkillsNames(e_Skill.Mineria) = "Mineria"
178     SkillsNames(e_Skill.Herreria) = "Herreria"
180     SkillsNames(e_Skill.Carpinteria) = "Carpinteria"
182     SkillsNames(e_Skill.Alquimia) = "Alquimia"
184     SkillsNames(e_Skill.Sastreria) = "Sastreria"
...
194     ListaAtributos(e_Atributos.Constitucion) = "Constitucion"
196     ListaAtributos(e_Atributos.Carisma) = "Carisma"
    
200     IniPath = App.Path & "\"
    
        'Bordes del mapa
'202     MinXBorder = 1 + (XWindow \ 2)
'204     MaxXBorder = Mapinfo(Map).Width - (XWindow \ 2)
'206     MinYBorder = 1 + (YWindow \ 2)
'208     MaxYBorder = Mapinfo(Map).Height - (YWindow \ 2)
...
176     frmCargando.Label1(2).Caption = "Cargando Objetos de Herreria"
178     Call LoadArmasHerreria
180     Call LoadArmadurasHerreria
    
182     frmCargando.Label1(2).Caption = "Cargando Objetos de Carpinteria"
...
226     Call LoadMapData
        ...
        frmCargando.Label1(2).Caption = "Spawneando Npcs"
        DoEvents
        Call LoadNpcSpawn
        Call InitNpcSpawn
...
312     If frmMain.Visible Then frmMain.txStatus.Caption = "Escuchando conexiones entrantes ..."
        ...
        frmCargando.Label1(2).Caption = "Cargando backup de zonas"
        DoEvents
        Call RestoreZonaBackup
...
Sub MostrarNumUsers()
        ...
        If NumUsers <> LastNumUsers Then 'Solo reenvio si cambio el numero sino mando mucho msg al pedo.
...
100     If Zona(UserList(UserIndex).ZonaId).Terreno <> "DUNGEON" Then
102         If MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).trigger <> 1 And MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).trigger <> 2 And MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).trigger < 10 Then Intemperie = True
        Else
...
112             If Zona(.ZonaId).Terreno = Nieve Then
...
108             Call Sanar(UserIndex, bEnviarStats_HP, IIf(.flags.Descansar, SanaIntervaloDescansar, SanaIntervaloSinDescansar))
        ...
        'El Yind, la energia ahora nunca baja si llueve o estas desnudo sino q sube pero mas lento.
        Multiplicador = 1
        
        If .flags.Desnudo = 1 Then Multiplicador = Multiplicador * 4
        If Lloviendo And Intemperie(UserIndex) Then Multiplicador = Multiplicador * 3
                                
110     If Not HambreOSed Then
112         Call RecStamina(UserIndex, bEnviarStats_STA, IIf(.flags.Descansar, StaminaIntervaloDescansar, StaminaIntervaloSinDescansar) * Multiplicador)
        Else
...
100         With UserList(UserIndex)
102             trigger = MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).trigger
...
120           If .ChatCombate = 1 Then
                  ' "El veneno te ha causado Â¬1 puntos de daño."
...
112                 If .ChatCombate = 1 Then
                        ' "El fuego te ha causado Â¬1 puntos de daño."
...
102     If MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).trigger = 1 And MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).trigger = 2 And MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).trigger = 4 Then Exit Sub
...
224                         X = .flags.PortalX
226                         Y = .flags.PortalY
228                         Call SendToAreaByPos(.flags.PortalM, X, Y, PrepareMessageParticleFXToFloor(X, Y, e_ParticulasIndex.TpVerde, 0))
230                         Call SendToAreaByPos(.flags.PortalM, X, Y, PrepareMessageLightFXToFloor(X, Y, 0, 105))
        
232                         If MapData(Mapa).Tile(X, Y).TileExit.Map > 0 Then
234                             MapData(Mapa).Tile(X, Y).TileExit.Map = 0
236                             MapData(Mapa).Tile(X, Y).TileExit.X = 0
238                             MapData(Mapa).Tile(X, Y).TileExit.Y = 0
        
                            End If
        
240                         MapData(Mapa).Tile(X, Y).Particula = 0
242                         MapData(Mapa).Tile(X, Y).TimeParticula = 0
244                         MapData(Mapa).Tile(X, Y).Particula = 0
246                         MapData(Mapa).Tile(X, Y).TimeParticula = 0
248                         .flags.Portal = 0
250                         .flags.PortalM = 0
252                         .flags.PortalY = 0
254                         .flags.PortalX = 0
...
290                         Call WriteConsoleMsg(i, "Gracias por jugar Argentum United.", e_FontTypeNames.FONTTYPE_INFO)
...
104     Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor Â» Grabando Personajes", e_FontTypeNames.FONTTYPE_SERVER))
...
122     Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor Â» Personajes Grabados", e_FontTypeNames.FONTTYPE_SERVER))
...
102     Tilde = Replace$(Tilde, "Ã", "A")
104     Tilde = Replace$(Tilde, "Ã‰", "E")
106     Tilde = Replace$(Tilde, "Ã", "I")
108     Tilde = Replace$(Tilde, "Ã“", "O")
110     Tilde = Replace$(Tilde, "Ãš", "U")
