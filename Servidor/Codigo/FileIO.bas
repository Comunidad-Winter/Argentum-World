...
124             .ModApunalar = val(BalanceIni.GetValue("MODAPUNALAR", SearchVar))
...
100     If frmMain.Visible Then frmMain.txStatus.Caption = "Cargando base de datos de los objetos."
        ...
        Dim Etapa As String ' util para debug
        
        If Not FileExist(DatPath & "Obj.dat", vbArchive) Then
            MsgBox "El archivo de objetos " & DatPath & "Obj.dat no existe en el path acordado.", vbCritical + vbOKOnly
            End
        End If
        
        
        Etapa = "leyendo el archivo de objetos desde " & DatPath & "Obj.dat"
...
114         .value = 0
        ...
        Etapa = "iniciando la lectura de objetos desde " & DatPath & "Obj.dat (total de objetos a cargar: " & NumObjDatas & ")"
...
422                 Case e_OBJType.otDanyoMagico
...
434                     .LingoteIndex = val(Leer.GetValue(ObjKey, "LingoteIndex"))
                ...
                Etapa = "leyendo datos de complemento"
...
646     Set Leer = Nothing
        ...
        Etapa = "leyendo tesoro"
648     Call InitTesoro

        Etapa = "leyendo regalo"
650     Call InitRegalo

        Exit Sub

ErrHandler:
652     MsgBox "Ha ocurrido un error cargando el Objeto " & Object & ", mientras se encontraba " & Etapa & "..." & vbCrLf & "Mensaje: " & Err.Number & ": " & Err.Description
End Sub
...
114     ReDim MapData(1 To NumMaps) As t_MapTile
...
120         frmCargando.ToMapLbl = Map & "/" & NumMaps
            ...
            If FileExist(MapPath & "Mapa" & Map & ".csm", vbArchive) Then
...
116     Get #fh, , MapDat
        ...
        ReDim MapData(Map).Tile(1 To MapSize.Width, 1 To MapSize.Height)
        
        Mapinfo(Map).Width = MapSize.Width
        Mapinfo(Map).Height = MapSize.Height
...
128                 MapData(Map).Tile(Blqs(i).X, Blqs(i).Y).Blocked = Blqs(i).Lados
130             Next i
...
144                 MapData(Map).Tile(X, Y).Graphic(1) = L1(i).GrhIndex
...
148                     MapData(Map).Tile(X, Y).Blocked = MapData(Map).Tile(X, Y).Blocked Or FLAG_AGUA
                    End If
...
164                 MapData(Map).Tile(X, Y).Graphic(2) = L2(i).GrhIndex
                
166                 MapData(Map).Tile(X, Y).Blocked = MapData(Map).Tile(X, Y).Blocked And Not FLAG_AGUA
...
182                 MapData(Map).Tile(X, Y).Graphic(3) = L3(i).GrhIndex
                
184                 If EsArbol(L3(i).GrhIndex) Then
186                     MapData(Map).Tile(X, Y).Blocked = MapData(Map).Tile(X, Y).Blocked Or FLAG_ARBOL
                    End If
188             Next i
...
198                 MapData(Map).Tile(L4(i).X, L4(i).Y).Graphic(4) = L4(i).GrhIndex
200             Next i
...
214                 MapData(Map).Tile(X, Y).trigger = Triggers(i).trigger

                    ' Trigger detalles en agua
216                 If Triggers(i).trigger = e_Trigger.DETALLEAGUA Then
                        ' Vuelvo a poner flag agua
218                     MapData(Map).Tile(X, Y).Blocked = MapData(Map).Tile(X, Y).Blocked Or FLAG_AGUA
                    End If
                    
217                 If Triggers(i).trigger = e_Trigger.VALIDONADO Or Triggers(i).trigger = e_Trigger.NADOCOMBINADO Then
                        ' Vuelvo a poner flag agua
219                     MapData(Map).Tile(X, Y).Blocked = MapData(Map).Tile(X, Y).Blocked Or FLAG_AGUA
                    End If
...
230                 MapData(Map).Tile(Particulas(i).X, Particulas(i).Y).ParticulaIndex = Particulas(i).Particula
232                 MapData(Map).Tile(Particulas(i).X, Particulas(i).Y).ParticulaIndex = 0
234             Next i
...
244                 MapData(Map).Tile(Luces(i).X, Luces(i).Y).Luz.Color = Luces(i).Color
246                 MapData(Map).Tile(Luces(i).X, Luces(i).Y).Luz.Rango = Luces(i).Rango
248                 MapData(Map).Tile(Luces(i).X, Luces(i).Y).Luz.Color = 0
250                 MapData(Map).Tile(Luces(i).X, Luces(i).Y).Luz.Rango = 0
252             Next i
...
262                 MapData(Map).Tile(Objetos(i).X, Objetos(i).Y).ObjInfo.ObjIndex = Objetos(i).ObjIndex

264                 Select Case ObjData(Objetos(i).ObjIndex).OBJType

                        Case e_OBJType.otYacimiento, e_OBJType.otArboles
266                         MapData(Map).Tile(Objetos(i).X, Objetos(i).Y).ObjInfo.Amount = ObjData(Objetos(i).ObjIndex).VidaUtil
268                         MapData(Map).Tile(Objetos(i).X, Objetos(i).Y).ObjInfo.data = &H7FFFFFFF ' Ultimo uso = Max Long

270                     Case Else
272                         MapData(Map).Tile(Objetos(i).X, Objetos(i).Y).ObjInfo.Amount = Objetos(i).ObjAmmount
...
294                         MapData(Map).Tile(NPCs(i).X, NPCs(i).Y).NpcIndex = NpcIndex
    
296                         NpcList(NpcIndex).Pos.Map = Map
298                         NpcList(NpcIndex).Pos.X = NPCs(i).X
300                         NpcList(NpcIndex).Pos.Y = NPCs(i).Y
                            NpcList(NpcIndex).ZonaId = MapData(Map).Tile(NpcList(NpcIndex).Pos.X, NpcList(NpcIndex).Pos.Y).ZonaId
...
306                             MapData(Map).Tile(NPCs(i).X, NPCs(i).Y).NpcIndex = 0
...
322                 MapData(Map).Tile(TEs(i).X, TEs(i).Y).TileExit.Map = TEs(i).DestM
324                 MapData(Map).Tile(TEs(i).X, TEs(i).Y).TileExit.X = TEs(i).DestX
326                 MapData(Map).Tile(TEs(i).X, TEs(i).Y).TileExit.Y = TEs(i).DestY
328             Next i
...
139     DebugMaps = val(Lector.GetValue("init", "DebugMaps"))
...
290     Ciudades(e_Ciudad.cNix) = CityNix
292     Ciudades(e_Ciudad.cUllathorpe) = CityUllathorpe
294     Ciudades(e_Ciudad.cBanderbill) = CityBanderbill
296     Ciudades(e_Ciudad.cLindos) = CityLindos
298     Ciudades(e_Ciudad.cArghal) = CityArghal
300     Ciudades(e_Ciudad.cArkhein) = CityArkhein
...
Sub LoadUser(ByVal UserIndex As Integer)
        ...
        Call SendLoadCharacter(UserIndex, UserList(UserIndex).AccountId, UserList(UserIndex).Name)
        Exit Sub
...
Sub SaveUser(ByVal UserIndex As Integer, Optional ByVal Logout As Boolean = False)
        ...
        Call SendSaveCharacter(UserIndex, Logout)
