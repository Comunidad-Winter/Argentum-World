...
100     If MapData(Map).Tile(X, Y).UserIndex <> 0 Or MapData(Map).Tile(X, Y).NpcIndex <> 0 Then
                    
            ' Se teletransporta a la misma pos a la que estaba
102         If MapData(Map).Tile(X, Y).UserIndex = UserIndex Then Exit Sub
...
110                     If MapData(Map).Tile(tX, tY).UserIndex = 0 And MapData(Map).Tile(tX, tY).NpcIndex = 0 Then
...
130             OtherUserIndex = MapData(Map).Tile(X, Y).UserIndex
...
Private Function CheckMapRestrictions(ByVal UserIndex As Integer, ByRef Pos As t_WorldPos) As Boolean
...
106         If Zona(ZonaId).Newbie And Not EsNewbie(UserIndex) Then
...
114         If (Zona(ZonaId).Faccion = 1 Or Zona(ZonaId).Faccion = 3) And (Status(UserIndex) = 0 Or Status(UserIndex) = 2) Then
...
122         If (Zona(ZonaId).Faccion = 2 Or Zona(ZonaId).Faccion = 4) And (Status(UserIndex) = 1 Or Status(UserIndex) = 3) Then
...
130         If Zona(ZonaId).SoloClanes And .GuildIndex <= 0 Then
...
138         If Zona(ZonaId).MinLevel <> 0 And .Stats.ELV < Zona(ZonaId).MinLevel Then
140             If .flags.UltimoMensaje <> 105 Then
142                 Call WriteConsoleMsg(UserIndex, "Necesitas ser al menos nivel " & Zona(ZonaId).MinLevel & " para entrar a este mapa.", e_FontTypeNames.FONTTYPE_INFO)
144                 .flags.UltimoMensaje = 105
                End If
                Exit Function
            End If

146         If Zona(ZonaId).MaxLevel <> 0 And .Stats.ELV >= Zona(ZonaId).MaxLevel Then
148             If .flags.UltimoMensaje <> 106 Then
150                 Call WriteConsoleMsg(UserIndex, "Sólo los personajes inferiores a nivel " & Zona(ZonaId).MaxLevel & " pueden entrar a este mapa.", e_FontTypeNames.FONTTYPE_INFO)
...
Public Sub DoTileEvents(ByVal UserIndex As Integer, ByVal Map As Integer, ByVal X As Integer, ByVal Y As Integer)
        ...
        'Uses: Zona(.zonaid).Restringir = "NEWBIE" (newbies), "ARMADA", "CAOS", "FACCION" or "NO".
        '***************************************************
...
102         If InMapBounds(Map, X, Y) Then
                ...
                If MapData(Map).Tile(X, Y).trigger = AUTORESU Then
                    Call ResucitarOCurar(UserIndex)
                End If
        
104             If MapData(Map).Tile(X, Y).ObjInfo.ObjIndex > 0 Then
106                 EsTeleport = ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex).OBJType = e_OBJType.otTeleport
                End If
    
108             If (MapData(Map).Tile(X, Y).TileExit.Map > 0) And (MapData(Map).Tile(X, Y).TileExit.Map <= NumMaps) Then
    
                    ' WyroX: Restricciones de mapas
110                 If CheckMapRestrictions(UserIndex, MapData(Map).Tile(X, Y).TileExit) Then
112                     If Zona(MapData(MapData(Map).Tile(X, Y).TileExit.Map).Tile(MapData(Map).Tile(X, Y).TileExit.X, MapData(Map).Tile(X, Y).TileExit.Y).ZonaId).Interdimensional = 1 And Zona(.ZonaId).Interdimensional = 0 Then
114                         .flags.ReturnPos = .Pos
                        End If
                        
                        Dim destPos As t_WorldPos
                        
                        destPos.Map = MapData(Map).Tile(X, Y).TileExit.Map
                        If EsTeleport Then
                            destPos.X = RandomNumber(MapData(Map).Tile(X, Y).TileExit.X - ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex).Radio, MapData(Map).Tile(X, Y).TileExit.X + ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex).Radio)
                            destPos.Y = RandomNumber(MapData(Map).Tile(X, Y).TileExit.Y - ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex).Radio, MapData(Map).Tile(X, Y).TileExit.Y + ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex).Radio)
                        Else
                            destPos.X = MapData(Map).Tile(X, Y).TileExit.X
                            destPos.Y = MapData(Map).Tile(X, Y).TileExit.Y
                        End If
...
158             ElseIf MapData(Map).Tile(X, Y).TileExit.Map < 0 Then
160                 If .flags.ReturnPos.Map <> 0 Then
    
    
                        'El Yind corrijo 1 tile para que cuando salga de una puerta con este tipo de exit salga en la misma linea y no 1 tile mal.
                        Select Case .Char.Heading
                            Case e_Heading.NORTH
                                .flags.ReturnPos.Y = .flags.ReturnPos.Y - 1
                            Case e_Heading.SOUTH
                                .flags.ReturnPos.Y = .flags.ReturnPos.Y + 1
                            Case e_Heading.WEST
                                .flags.ReturnPos.X = .flags.ReturnPos.X - 1
                            Case e_Heading.EAST
                                .flags.ReturnPos.X = .flags.ReturnPos.X + 1
                        End Select
...
100     If X > UserList(UserIndex).Pos.X - HALF_X And X < UserList(UserIndex).Pos.X + HALF_X Then
102         If Y > UserList(UserIndex).Pos.Y - HALF_Y And Y < UserList(UserIndex).Pos.Y + HALF_Y Then
...
100     If X > NpcList(NpcIndex).Pos.X - HALF_X And X < NpcList(NpcIndex).Pos.X + HALF_X Then
102         If Y > NpcList(NpcIndex).Pos.Y - HALF_Y And Y < NpcList(NpcIndex).Pos.Y + HALF_Y Then
...
100     If (Map <= 0 Or Map > NumMaps) Or X < 1 Or X > Mapinfo(Map).Width Or Y < 1 Or Y > Mapinfo(Map).Height Then
...
112                 If LegalPos(nPos.Map, tX, tY) And MapData(nPos.Map).Tile(tX, tY).TileExit.Map = 0 Then
114                     nPos.X = tX
116                     nPos.Y = tY
                        '¿Hay objeto?
...
102     If X < 1 Or X > Mapinfo(Map).Width Then Exit Function
        
104     If Y < 1 Or Y > Mapinfo(Map).Height Then Exit Function
        
106     With MapData(Map).Tile(X, Y)
...
104     ElseIf MapData(Map).Tile(X, Y).TileExit.Map > 0 Then
106         LegalPosNPC = False
        
        Else

108         If AguaValida = 0 Then
110             LegalPosNPC = (MapData(Map).Tile(X, Y).Blocked And e_Block.ALL_SIDES) <> e_Block.ALL_SIDES And (MapData(Map).Tile(X, Y).UserIndex = 0) And (MapData(Map).Tile(X, Y).NpcIndex = 0) And (MapData(Map).Tile(X, Y).trigger <> e_Trigger.POSINVALIDA Or IsPet) And (MapData(Map).Tile(X, Y).Blocked And FLAG_AGUA) = 0
            Else
112             LegalPosNPC = (MapData(Map).Tile(X, Y).Blocked And e_Block.ALL_SIDES) <> e_Block.ALL_SIDES And (MapData(Map).Tile(X, Y).UserIndex = 0) And (MapData(Map).Tile(X, Y).NpcIndex = 0) And (MapData(Map).Tile(X, Y).trigger <> e_Trigger.POSINVALIDA Or IsPet)
            End If
...
108         UserList(UserIndex).flags.TargetY = Y
            ...
            '¿Es un obj?
110         If MapData(Map).Tile(X, Y).ObjInfo.ObjIndex > 0 Then
                'Informa el nombre
...
120         ElseIf MapData(Map).Tile(X + 1, Y).ObjInfo.ObjIndex > 0 Then

                'Informa el nombre
122             If ObjData(MapData(Map).Tile(X + 1, Y).ObjInfo.ObjIndex).OBJType = e_OBJType.otPuertas Then
...
132         ElseIf MapData(Map).Tile(X + 1, Y + 1).ObjInfo.ObjIndex > 0 Then

134             If ObjData(MapData(Map).Tile(X + 1, Y + 1).ObjInfo.ObjIndex).OBJType = e_OBJType.otPuertas Then
                    'Informa el nombre
...
144         ElseIf MapData(Map).Tile(X, Y + 1).ObjInfo.ObjIndex > 0 Then

146             If ObjData(MapData(Map).Tile(X, Y + 1).ObjInfo.ObjIndex).OBJType = e_OBJType.otPuertas Then
                    'Informa el nombre
...
158             UserList(UserIndex).flags.TargetObj = MapData(Map).Tile(UserList(UserIndex).flags.TargetObjX, UserList(UserIndex).flags.TargetObjY).ObjInfo.ObjIndex

160             If MostrarCantidad(UserList(UserIndex).flags.TargetObj) Then

162                 Call WriteConsoleMsg(UserIndex, "O*" & UserList(UserIndex).flags.TargetObj & "* - " & MapData(UserList(UserIndex).flags.TargetObjMap).Tile(UserList(UserIndex).flags.TargetObjX, UserList(UserIndex).flags.TargetObjY).ObjInfo.Amount & "", e_FontTypeNames.FONTTYPE_INFO)
                Else
...
168                     Call WriteConsoleMsg(UserIndex, ObjData(UserList(UserIndex).flags.TargetObj).Name & " - (Minerales disponibles: " & MapData(Map).Tile(UserList(UserIndex).flags.TargetObjX, UserList(UserIndex).flags.TargetObjY).ObjInfo.Amount & ")", e_FontTypeNames.FONTTYPE_INFO)
...
174                     Call WriteConsoleMsg(UserIndex, ObjData(UserList(UserIndex).flags.TargetObj).Name & " - (Recursos disponibles: " & MapData(Map).Tile(UserList(UserIndex).flags.TargetObjX, UserList(UserIndex).flags.TargetObjY).ObjInfo.Amount & ")", e_FontTypeNames.FONTTYPE_INFO)
                    
176                 ElseIf ObjData(UserList(UserIndex).flags.TargetObj).OBJType = e_OBJType.otTeleport Then

                        With MapData(Map).Tile(X, Y).TileExit
178                         If .Map > 0 Then
                                Dim map_name As String
                                map_name = Zona(MapData(.Map).Tile(.X, .Y).ZonaId).Zona_name
180                             If LenB(map_name) <> 0 Then
182                                 Call WriteConsoleMsg(UserIndex, "Portal a " & map_name, e_FontTypeNames.FONTTYPE_INFO)
                                Else
...
188                     Call WriteConsoleMsg(UserIndex, "O*" & UserList(UserIndex).flags.TargetObj & "*", e_FontTypeNames.FONTTYPE_INFO)
            ...
            Dim yy       As Integer
            '¿Es un personaje?
190         If Y + 1 <= Mapinfo(Map).Height Then
192             If MapData(Map).Tile(X, Y + 1).UserIndex > 0 Then
194                 TempCharIndex = MapData(Map).Tile(X, Y + 1).UserIndex
                    yy = Y + 1
196                 FoundChar = 1

                End If

198             If MapData(Map).Tile(X, Y + 1).NpcIndex > 0 Then
200                 TempCharIndex = MapData(Map).Tile(X, Y + 1).NpcIndex
202                 FoundChar = 2
...
206             If MapData(Map).Tile(X, Y).UserIndex > 0 Then
208                 TempCharIndex = MapData(Map).Tile(X, Y).UserIndex
                    yy = Y
210                 FoundChar = 1

                End If

212             If MapData(Map).Tile(X, Y).NpcIndex > 0 Then
214                 TempCharIndex = MapData(Map).Tile(X, Y).NpcIndex
216                 FoundChar = 2
...
218         If FoundChar = 1 Then '  ¿Encontro un Usuario?
                ...
                If UserList(TempCharIndex).Pos.X <> X Or UserList(TempCharIndex).Pos.Y <> yy Then
                    'Mapblock bugeado
                    MapData(Map).Tile(X, yy).UserIndex = 0
...
100     If MapData(Map).Tile(X, Y).ObjInfo.ObjIndex > 0 Then
102         HayPuerta = (ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex).OBJType = e_OBJType.otPuertas) And ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex).Cerrada = 1 And (ObjData(MapData(Map).Tile(X, Y).ObjInfo.ObjIndex).Llave = 0)
        End If
End Function
