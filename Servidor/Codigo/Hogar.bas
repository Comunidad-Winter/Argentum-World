...
Public Ciudades(1 To NUMCIUDADES)         As t_CityWorldPos
...
118         tX = Ciudades(.Hogar).Resux
120         tY = Ciudades(.Hogar).Resuy
122         tMap = Ciudades(.Hogar).MapaResu
        
124         Call FindLegalPos(UserIndex, tMap, tX, tY)
...
132         .Counters.goHome = 0
            ...
            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageParticleFX(.Char.charindex, e_ParticulasIndex.Runa, 0, True, , .Pos.X, .Pos.Y))
            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageBarFx(.Char.charindex, 0, e_AccionBarra.Hogar))
