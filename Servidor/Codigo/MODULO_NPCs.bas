...
142         UserList(UserIndex).Stats.NPCsMatados = UserList(UserIndex).Stats.NPCsMatados + 1
            
144         If MiNPC.MaestroUser > 0 Then Exit Sub

            If MiNPC.Stats.MaxHp >= 20000 Then Call SendCharacterEvent(UserIndex, e_EventType.KillNpc, UserList(UserIndex).Name & " mata " & MiNPC.Name & " en " & MiNPC.Pos.Map & "," & MiNPC.Pos.X & "," & MiNPC.Pos.Y, MiNPC.Numero, 1, 0)
...
100     TestSpawnTrigger = MapData(Map).Tile(X, Y).trigger < 1 Or (MapData(Map).Tile(X, Y).trigger > 3 And MapData(Map).Tile(X, Y).trigger < 9) 'Martin ver que pasa con el trigger 7
...
Public Function CrearNPC(NroNPC As Integer, NpcSpawnId As Integer, OrigPos As t_WorldPos, Optional ByVal CustomHead As Integer)
...
122             .Pos = OrigPos
            ...
            Else
                ...
                If NpcSpawnId = 0 Then
                    ...
                    Call LogError("Se intento CrearNpc con SpawnNpcId 0 NroNpc:" & NroNPC)
                    Exit Function
                End If
                
                
                Do
124                 .Pos.Map = NpcSpawn(NpcSpawnId).Map
126                 .Pos.X = RandomNumber(NpcSpawn(NpcSpawnId).X, NpcSpawn(NpcSpawnId).X2)   'Obtenemos posicion al azar en x
128                 .Pos.Y = RandomNumber(NpcSpawn(NpcSpawnId).Y, NpcSpawn(NpcSpawnId).Y2)   'Obtenemos posicion al azar en y
...
138                 .Pos.Map = NpcSpawn(NpcSpawnId).Map
140                 .Pos.X = (NpcSpawn(NpcSpawnId).X2 - NpcSpawn(NpcSpawnId).X + 1) \ 2
142                 .Pos.Y = (NpcSpawn(NpcSpawnId).Y2 - NpcSpawn(NpcSpawnId).Y + 1) \ 2
                    
144                 .Pos = ClosestLegalPosNPC(NpcIndex, (NpcSpawn(NpcSpawnId).X2 - NpcSpawn(NpcSpawnId).X + 1) \ 2, , True)
                    
                    ' Si sigue fallando
146                 If .Pos.X = 0 And .Pos.Y = 0 Then
                        ' Hacemos una última búsqueda exhaustiva, ignorando los usuarios
148                     .Pos.Map = NpcSpawn(NpcSpawnId).Map
150                     .Pos.X = (NpcSpawn(NpcSpawnId).X2 - NpcSpawn(NpcSpawnId).X + 1) \ 2
152                     .Pos.Y = (NpcSpawn(NpcSpawnId).Y2 - NpcSpawn(NpcSpawnId).Y + 1) \ 2
                        
154                     .Pos = ClosestLegalPosNPC(NpcIndex, (NpcSpawn(NpcSpawnId).X2 - NpcSpawn(NpcSpawnId).X + 1) \ 2, True)
                        
                        ' Si falló, borramos el NPC y salimos
156                     If .Pos.X = 0 And .Pos.Y = 0 Then
                            Call LogError("No se logro spawnear npc en CrearNpc con NpcSpawnId " & NpcSpawnId & "  NroNpc:" & NroNPC)
...
110         MapData(Map).Tile(X, Y).NpcIndex = NpcIndex
...
154                     Simbolo = 3
                ...
                If UserList(sndIndex).flags.Muerto = 1 And Zona(UserList(sndIndex).ZonaId).Segura = 0 Then
...
108             If LastChar <= 1 Then Exit Do
        ...
        'Quitamos del mapa
110     MapData(NpcList(NpcIndex).Pos.Map).Tile(NpcList(NpcIndex).Pos.X, NpcList(NpcIndex).Pos.Y).NpcIndex = 0
...
112             UserIndex = MapData(.Pos.Map).Tile(nPos.X, nPos.Y).UserIndex
...
118                     MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).UserIndex = 0
120                     .Pos.X = NpcList(NpcIndex).Pos.X
122                     .Pos.Y = NpcList(NpcIndex).Pos.Y
124                     MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).UserIndex = UserIndex
                        .ZonaId = ZonaByPos(.Pos)
...
150             MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).NpcIndex = 0
152             .Pos = nPos
154             .Char.Heading = nHeading
156             MapData(.Pos.Map).Tile(nPos.X, nPos.Y).NpcIndex = NpcIndex
                .ZonaId = ZonaByPos(.Pos)
...
100     nIndex = OpenNPC(NpcIndex, False)   'Conseguimos un indice
...
128     Y = NpcList(nIndex).Pos.Y
        NpcList(nIndex).ZonaId = MapData(Map).Tile(X, Y).ZonaId
...
100     If (MiNPC.flags.Respawn = 0) Then Call CrearNPC(MiNPC.Numero, MiNPC.SpawnNpcId, MiNPC.Orig)
...
138         .Char.Head = val(Leer.GetValue("NPC" & NpcNumber, "Head"))
            ...
            If .Char.Head < 0 Then
                Select Case .Char.Head
                    Case -1 'Humanos
                        .Char.Head = RandomNumber(1, 41)
                    Case -2 'Elfo
                        .Char.Head = RandomNumber(101, 132)
                    Case -3 'Elfo oscuro
                        .Char.Head = RandomNumber(200, 229)
                    Case -4 'Enano
                        .Char.Head = RandomNumber(300, 329)
                    Case -5 'Gnomo
                        .Char.Head = RandomNumber(400, 429)
                    Case -6 'Orco
                        .Char.Head = RandomNumber(500, 529)
                End Select
            End If
...
252             .Invent.Object(LoopC).Amount = val(ReadField(2, ln, 45))
                .Invent.Object(LoopC).Crucial = ObjData(val(ReadField(1, ln, 45))).Crucial
254         Next LoopC
...
306             .flags.Respawn = val(Leer.GetValue("NPC" & NpcNumber, "ReSpawn"))
            Else
308             .flags.Respawn = 1 'Esta logica esta invertida.. si es 1 significa que no respawnea
            End If
...
116         NpcList(NpcIndex).Pos = NuevaPos
            NpcList(NpcIndex).ZonaId = ZonaByPos(NuevaPos)
