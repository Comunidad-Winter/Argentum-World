...
Private Const NPC_CENTINELA_TIERRA As Integer = 498  'Ãndice del NPC en el .dat

Private Const NPC_CENTINELA_AGUA   As Integer = 499    'Ãdem anterior, pero en mapas de agua

Public CentinelaNPCIndex           As Integer                'Ãndice del NPC en el servidor
...
102         Name = UserList(Centinela.RevisandoUserIndex).Name
            ...
            Call SendAddPenalty(0, Name, 20, "CENTINELA : Encarcelado por no responder. ")
...
114             If modMatematicas.Distancia(NpcList(CentinelaNPCIndex).Pos, UserList(Centinela.RevisandoUserIndex).Pos) > 2 Then
...
106     If (MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).Blocked And FLAG_AGUA) <> 0 Then
