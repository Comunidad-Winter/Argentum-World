...
122       Call WriteLocaleMsg(UserIndex, "32", e_FontTypeNames.FONTTYPE_FIGHT, NpcList(NpcIndex).Name & "Â¬" & DañoStr)
...
156       .Stats.MinHp = .Stats.MinHp - Daño
          ...
          'Call WriteLocaleMsg(UserIndex, "34", e_FontTypeNames.FONTTYPE_FIGHT, NpcList(NpcIndex).name & "Â¬" & DañoStr)
...
124                     TargetMap = MapData(NpcList(NpcIndex).Pos.Map).Tile(X + PosCasteadax - mitadAreaRadio, PosCasteaday + Y - mitadAreaRadio)
...
114         If Zona(.ZonaId).SinMagia Then
...
130             If Zona(.ZonaId).Segura = 1 Or MapData(.Pos.Map).Tile(.Pos.X, .Pos.Y).trigger = e_Trigger.ZonaSegura Then
132                 Call WriteConsoleMsg(UserIndex, "En zona segura no puedes invocar criaturas.", e_FontTypeNames.FONTTYPE_INFO)
                ...
                If Zona(.ZonaId).SinMascotas Then
...
168                 If Zona(.ZonaId).Segura = 1 Then
...
192                             .MascotasIndex(i) = SpawnNpc(.MascotasType(i), targetPos, True, False, False, UserIndex)
...
118                     If MapData(PosCasteadaM).Tile(TempX, TempY).UserIndex > 0 Then

                            'hay un user
120                         If UserList(MapData(PosCasteadaM).Tile(TempX, TempY).UserIndex).flags.invisible = 1 And UserList(MapData(PosCasteadaM).Tile(TempX, TempY).UserIndex).flags.NoDetectable = 0 Then
122                             UserList(MapData(PosCasteadaM).Tile(TempX, TempY).UserIndex).flags.invisible = 0
124                             Call WriteConsoleMsg(MapData(PosCasteadaM).Tile(TempX, TempY).UserIndex, "Tu invisibilidad ya no tiene efecto.", e_FontTypeNames.FONTTYPE_INFOIAO)
126                             Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageSetInvisible(UserList(MapData(PosCasteadaM).Tile(TempX, TempY).UserIndex).Char.charindex, False, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y))
...
134             TargetMap = MapData(UserList(UserIndex).Pos.Map).Tile(X + PosCasteadax - CInt(Hechizos(h).AreaRadio / 2), PosCasteaday + Y - CInt(Hechizos(h).AreaRadio / 2))
...
108     If MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).flags.TargetX, UserList(UserIndex).flags.TargetY).ObjInfo.Amount > 0 Or (MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).flags.TargetX, UserList(UserIndex).flags.TargetY).Blocked And e_Block.ALL_SIDES) <> e_Block.ALL_SIDES Or MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).flags.TargetX, UserList(UserIndex).flags.TargetY).TileExit.Map > 0 Or UserList(UserIndex).flags.TargetUser <> 0 Then
110         b = False
...
102     If MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).flags.TargetX, UserList(UserIndex).flags.TargetY).ObjInfo.Amount > 0 Or MapData(UserList(UserIndex).Pos.Map).Tile(UserList(UserIndex).flags.TargetX, UserList(UserIndex).flags.TargetY).Blocked Then
104         b = False
...
162         If Zona(UserList(tU).ZonaId).SinInvi Then
...
114             Call WriteLocaleMsg(UserIndex, "388", e_FontTypeNames.FONTTYPE_FIGHT, "la criaturaÂ¬" & DañoStr)
...
184             Call WriteLocaleMsg(UserIndex, "389", e_FontTypeNames.FONTTYPE_FIGHT, "la criaturaÂ¬" & DañoStr)
            End If
...
364         ElseIf trigger = TRIGGER6_PROHIBE And Zona(UserList(UserIndex).ZonaId).Segura <> 0 Then
366             b = False
                Exit Sub
            End If
...
380             Call WriteLocaleMsg(UserIndex, "388", e_FontTypeNames.FONTTYPE_FIGHT, UserList(tempChr).Name & "Â¬" & DañoStr)

                'Call WriteConsoleMsg(tempChr, UserList(UserIndex).name & " te ha restaurado " & Daño & " puntos de vida.", e_FontTypeNames.FONTTYPE_FIGHT)
382             Call WriteLocaleMsg(tempChr, "32", e_FontTypeNames.FONTTYPE_FIGHT, UserList(UserIndex).Name & "Â¬" & DañoStr)
            Else
...
442             Daño = Daño - Porcentaje(Daño, PorcentajeRM)
            ...
            Call DañoExtraPorCombo(UserIndex, tempChr, Daño, tipoDanio.t_magia)
...
456         Call WriteLocaleMsg(UserIndex, "389", e_FontTypeNames.FONTTYPE_FIGHT, UserList(tempChr).Name & "Â¬" & DañoStr)

            'Call WriteConsoleMsg(tempChr, UserList(UserIndex).name & " te ha quitado " & Daño & " puntos de vida.", e_FontTypeNames.FONTTYPE_FIGHT)
458         Call WriteLocaleMsg(tempChr, "34", e_FontTypeNames.FONTTYPE_FIGHT, UserList(UserIndex).Name & "Â¬" & DañoStr)
...
304                 Daño = Daño - Porcentaje(Daño, ObjData(UserList(tempChr).Invent.CascoEqpObjIndex).ResistenciaMagica)
                ...
                ' Resistencia magica de la clase
...
Private Sub AreaHechizo(UserIndex As Integer, NpcIndex As Integer, X As Integer, Y As Integer, NPC As Boolean)
