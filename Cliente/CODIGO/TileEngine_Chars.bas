...
Public Sub Char_Move_by_Head(ByVal charindex As Integer, ByVal nHeading As E_Heading)
    ...
    With charlist(charindex)
        ...
        MapData(rrX(nX), rrY(nY)).charindex = charindex
        .Pos.x = nX
        .Pos.y = nY
        
        If MapData(rrX(x), rrY(y)).charindex = charindex Then
        ...
        If MapData(rrX(nX), rrY(nY)).ObjGrh.GrhIndex = 26940 Or MapData(rrX(nX), rrY(nY)).Trigger = ESCALERA Then
...
Public Sub Char_Move_by_Pos(ByVal charindex As Integer, ByVal nX As Integer, ByVal nY As Integer)
    ...
    With charlist(charindex)
        ...
        If MapData(rrX(nX), rrY(nY)).ObjGrh.GrhIndex = 26940 Then
            .Heading = E_Heading.NORTH
        Else
            .Heading = nHeading
        End If
...
Public Function EstaPCarea(ByVal charindex As Integer) As Boolean
    ...
    With charlist(charindex).Pos
        EstaPCarea = .x > UserPos.x - 12 And .x < UserPos.x + 12 And .y > UserPos.y - 10 And .y < UserPos.y + 10
...
Public Function EstaEnArea(ByVal x As Integer, ByVal y As Integer) As Boolean
    ...
    EstaEnArea = x > UserPos.x - 12 And x < UserPos.x + 12 And y > UserPos.y - 10 And y < UserPos.y + 10
...
Public Function Get_PixelY_Of_Char(ByVal char_index As Integer) As Integer
    ...
    If Char_Check(char_index) Then
        Get_PixelY_Of_Char = (charlist(char_index).Pos.y - 2 - UserPos.y) * 32 + MainViewWidth / 2
...
Public Function Get_Pixelx_Of_Char(ByVal char_index As Integer) As Integer
    ...
    'Make sure it's a legal char_index
    If Char_Check(char_index) Then
        Get_Pixelx_Of_Char = (charlist(char_index).Pos.x - UserPos.x) * 32 + MainViewWidth / 2
...
Public Function Get_Pixelx_Of_XY(ByVal x As Integer) As Integer
    '*****************************************************************
    'Author: Pablo Mercavides
    '*****************************************************************
    'Make sure it's a legal char_index
    ...
    Get_Pixelx_Of_XY = (x - UserPos.x) * 32 + MainViewWidth / 2
...
Public Function Get_PixelY_Of_XY(ByVal y As Integer) As Integer
    '*****************************************************************
    'Author: Pablo Mercavides
    '*****************************************************************
    'Make sure it's a legal char_index
    ...
    Get_PixelY_Of_XY = (y - 2 - UserPos.y) * 32 + MainViewWidth / 2
