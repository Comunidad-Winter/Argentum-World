...
Public StartScreenY         As Integer
...
Public Sub Init_TileEngine()
    ...
    ' Redondeamos hacia arriba hasta el impar más cercano
    WindowTileWidth = (Ceil(MainViewWidth / 32) \ 2) * 2 + 1
    WindowTileHeight = (Ceil(MainViewHeight / 32) \ 2) * 2 + 1
    
    HalfWindowTileWidth = WindowTileWidth \ 2
    HalfWindowTileHeight = WindowTileHeight \ 2
    
    StartScreenX = (MainViewWidth - WindowTileWidth * 32) \ 2
    StartScreenY = (MainViewHeight - WindowTileHeight * 32) \ 2
    
    HalfConnectTileHeight = (frmConnect.render.ScaleHeight / 32) \ 2
    HalfConnectTileWidth = (frmConnect.render.ScaleWidth / 32) \ 2
    
    ' WyroX: Si vemos que hace falta agrandar el buffer, se hace, pero con cuidado porque mata los FPS
    With TileBufferSize
        .Left = 4
        .Top = 4
        .Right = 4
...
Sub ConvertCPtoTP(ByVal viewPortX As Integer, ByVal viewPortY As Integer, ByRef tX As Integer, ByRef tY As Integer)
  ...
  '  Debug.Print "viewportX: " & viewPortX & "- ViewportY: " & viewPortY
    If viewPortX < 0 Or viewPortX > MainViewWidth Then Exit Sub
    If viewPortY < 0 Or viewPortY > MainViewHeight Then Exit Sub

    tX = UserPos.x + viewPortX \ 32 - MainViewWidth \ 64
    tY = UserPos.y + viewPortY \ 32 - MainViewHeight \ 64
    ...
    If Not charlist(charindex).Navegando Then
        ...
        With charlist(charindex)
            ...
            If Not .Muerto And EstaPCarea(charindex) And .priv <= charlist(UserCharIndex).priv And charlist(UserCharIndex).Muerto = False Then
                ...
                If .Pie Then
                    If MapData(rrX(.Pos.x), rrY(.Pos.y)).Graphic(1).GrhIndex > 0 Then
                        FileNum = GrhData(MapData(rrX(.Pos.x), rrY(.Pos.y)).Graphic(1).GrhIndex).FileNum
...
Sub MoveScreen(ByVal nHeading As E_Heading)
    ...
    If tX < 1 Or tX > MapSize.Width Or tY < 1 Or tY > MapSize.Height Then
        Exit Sub
    Else
        ...
        Select Case nHeading
            ...
            Case E_Heading.NORTH
                For i = Int(AREA_DIM * -2.5) To Int(AREA_DIM * 2.5)
                    If UserMap = 1 Then
                        Call CargarTile(UserPos.x + i, UserPos.y - 24, DataMap1)
                    Else
                        Call CargarTile(UserPos.x + i, UserPos.y - 24, DataMap2)
                    End If
                Next i
            Case E_Heading.EAST
                x = 1
                For i = Int(AREA_DIM * -2.5) To Int(AREA_DIM * 2.5)
                    If UserMap = 1 Then
                        Call CargarTile(UserPos.x + 24, UserPos.y + i, DataMap1)
                    Else
                        Call CargarTile(UserPos.x + 24, UserPos.y + i, DataMap2)
                    End If
                Next i
            Case E_Heading.south
                y = 1
...
Public Function NearRoof(ByVal x As Integer, ByVal y As Integer) As eTrigger
    ...
    For lY = y - 1 To y + 1
        For lX = x - 1 To x + 1
            If lX >= 1 And lX <= MapSize.Width Then
                If lY >= 1 And lY <= MapSize.Height Then
                    If HayTecho(lX, lY) Then
                        NearRoof = MapData(rrX(lX), rrY(lY)).Trigger
                        Exit Function
                    End If
                End If
            End If
    ...
    For j = UserPos.x - 13 To UserPos.x + 13
        For k = UserPos.y - 15 To UserPos.y + 15
            ...
            If InMapBounds(j, k) Then
                If MapData(rrX(j), rrY(k)).ObjGrh.GrhIndex = GrhFogata Then
                    location.x = j
                    location.y = k
...
Public Function HayWavAmbiental(ByRef location As Position) As Boolean
    ...
    For j = UserPos.x - 13 To UserPos.x + 13
        For k = UserPos.y - 15 To UserPos.y + 15
            ...
            If InMapBounds(j, k) Then
                If MapData(rrX(j), rrY(k)).Trigger = 150 Then
                    location.x = j
                    location.y = k
...
Function LegalPos(ByVal x As Integer, ByVal y As Integer, ByVal Heading As E_Heading) As Boolean
    ...
    If MapData(rrX(x), rrY(y)).charindex > 0 Then
        With charlist(MapData(rrX(x), rrY(y)).charindex)
    ...
    If (MapData(rrX(x), rrY(y)).Blocked And 2 ^ (Heading - 1)) <> 0 Then Exit Function
    
    
    If UserMontado And MapData(rrX(x), rrY(y)).Trigger > 9 Then
        Exit Function
    End If

    If UserNadando And MapData(rrX(x), rrY(y)).Trigger = 8 Then
        LegalPos = True
        Exit Function
    End If
   '0 <>
    If UserNavegando <> ((MapData(rrX(x), rrY(y)).Blocked And FLAG_AGUA) <> 0 And (MapData(rrX(x), rrY(y)).Blocked And FLAG_COSTA) = 0) And MapData(rrX(x), rrY(y)).Trigger <> eTrigger.VALIDOPUENTE Then
        Exit Function
    End If
    
    If UserNadando And Not (MapData(rrX(x), rrY(y)).Trigger = eTrigger.DETALLEAGUA Or MapData(rrX(x), rrY(y)).Trigger = eTrigger.NADOCOMBINADO Or MapData(rrX(x), rrY(y)).Trigger = eTrigger.VALIDONADO) Then
        LegalPos = False
        Exit Function
    End If
    
    If UserNavegando And MapData(rrX(x), rrY(y)).Trigger = 8 And Not UserNadando And Not UserEstado = 1 Then
        If Not UserAvisadoBarca Then
            Call AddToConsole("¡Atención! El agua es poco profunda, tu barca podria romperse, solo puedes caminar.", 255, 255, 255, True, False, False)
    ...
    If UserNavegando And MapData(rrX(x), rrY(y)).Trigger = 11 And Not UserNadando And Not UserEstado = 1 Then
        If Not UserAvisadoBarca Then
            Call AddToConsole("¡Atención! El terreno es rocoso y tu barca podria romperse, solo puedes nadar.", 255, 255, 255, True, False, False)
            UserAvisadoBarca = True
...
End Function
Public Function GetTerrainHeight(x As Integer, y As Integer) As Integer
