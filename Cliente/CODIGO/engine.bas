...
Public MaskPixelShader As Long
...
Private Sub Engine_InitShaders()

        On Error GoTo Engine_InitShaderss_Err
    
        Dim s As String
    
        s = s & "ps.1.4" & vbNewLine
        s = s & "def c0, 1, 1, 1, 1" & vbNewLine
        s = s & "texld r0, t0" & vbNewLine
        s = s & "texld r1, t0" & vbNewLine
        s = s & "mul r0, r0, r1" & vbNewLine
        s = s & "mul r0, r0, c0" & vbNewLine

        Dim Buffer As D3DXBuffer, ErrLog As String

120     Set Buffer = DirectD3D8.AssembleShader(s, 0, Nothing, ErrLog)

        Dim ShaderArray() As Long, size As Long

125     size = Buffer.GetBufferSize() \ 4
130     ReDim ShaderArray(size - 1)
135     Call DirectD3D8.BufferGetData(Buffer, 0&, 4&, size, ShaderArray(0))

140     MaskPixelShader = DirectDevice.CreatePixelShader(ShaderArray(0))

145     Set Buffer = Nothing
    
        Exit Sub

Engine_InitShaderss_Err:
        Call RegistrarError(err.Number, err.Description, "engine.Engine_InitShaders", Erl)
        Resume Next
...
Private Sub Engine_InitExtras()
    ...
    ' Guardo el backbuffer y el zbuffer originales
...
ErrHandler:
    ...
    Call MsgBox("Ha ocurrido un error al iniciar el motor grafico." & vbNewLine & _
                "Asegúrate de tener los drivers gráficos actualizados y la librería DX8VB.dll registrada correctamente.", vbCritical, "ArgentumWorld")
...
Public Sub Engine_PauseScene()
    Call SpriteBatch.Flush
    Call DirectDevice.EndScene
End Sub

Public Sub Engine_ResumeScene()
    Call DirectDevice.BeginScene
    Call SpriteBatch.Begin
End Sub
...
Public Sub Draw_GrhIndexColor(ByVal grh_index As Long, ByVal x As Integer, ByVal y As Integer, ByRef Color() As RGBA)
    ...
    Call Batch_Textured_Box(x, y, GrhData(grh_index).pixelWidth, GrhData(grh_index).pixelHeight, GrhData(grh_index).sX, GrhData(grh_index).sY, GrhData(grh_index).FileNum, Color)
...
Public Sub Draw_Grh(ByRef grh As grh, ByVal x As Integer, ByVal y As Integer, ByVal center As Byte, ByVal animate As Byte, ByRef rgb_list() As RGBA, Optional ByVal Alpha As Boolean = False, Optional ByVal Angle As Single)
...
Sub Draw_GrhFX(ByRef grh As grh, ByVal x As Integer, ByVal y As Integer, ByVal center As Byte, ByVal animate As Byte, ByRef rgb_list() As RGBA, Optional ByVal Alpha As Boolean, Optional ByVal map_x As Integer = 1, Optional ByVal map_y As Integer = 1, Optional ByVal Angle As Single, Optional ByVal charindex As Integer)
...
Private Sub Draw_GrhSinLuz(ByRef grh As grh, ByVal x As Integer, ByVal y As Integer, ByVal center As Byte, ByVal animate As Byte, Optional ByVal Alpha As Boolean, Optional ByVal map_x As Integer = 1, Optional ByVal map_y As Integer = 1, Optional ByVal Angle As Single)
...
Public Sub render()
    ...
    If frmMain.Contadores.enabled Then
        ...
        If DrogaCounter > 0 Then
            ...
            PosY = PosY + 15 + ConsoleDialog.console_height
    ...
    If FadeInAlpha > 0 Then
        Call Engine_Draw_Box(0, 0, MainViewWidth, MainViewHeight, RGBA_From_Comp(0, 0, 0, FadeInAlpha))
...
Public Sub Mascota_Render(ByVal charindex As Integer, ByVal PixelOffsetX As Integer, ByVal PixelOffsetY As Integer)
    ...
    target_x = (MainViewWidth / 2) - ((UserPos.x - AddtoUserPos.x) - charlist(charindex).Pos.x) * 32 + charlist(charindex).MoveOffsetX
    target_y = (MainViewHeight / 2) - ((UserPos.y - AddtoUserPos.y) - charlist(charindex).Pos.y) * 32 + charlist(charindex).MoveOffsetY
    ...
    If dist_x > 40 Then
        mascota.PosX = mascota.PosX + (dir_vector.x / (MainViewWidth / 2)) * timerElapsedTime / 1000 * dist * 3  ' 256 como constante no le da aceleración.
        isAnimated = 1
    End If

    mascota.PosX = mascota.PosX - LastOffset2X
    If dist_y > 40 Then
        mascota.PosY = mascota.PosY + (dir_vector.y / (MainViewHeight / 2)) * timerElapsedTime / 1000 * dist * 3
        isAnimated = 1
    End If
...
Sub Char_TextRender(ByVal charindex As Integer, ByVal PixelOffsetX As Integer, ByVal PixelOffsetY As Integer, ByVal x As Integer, ByVal y As Integer)
    ...
    With charlist(charindex)
        ...
        If .dialog <> "" Then
            ...
            If .dialog_scroll Then
                ...
                Engine_Text_Render .dialog, PixelOffsetX + 14 - CInt(Engine_Text_Width(.dialog, True) / 2), PixelY + .Body.HeadOffset.y - Engine_Text_Height(.dialog, True) + .dialog_offset_counter_y, temp_array, 1, True, MapData(rrX(x), rrY(y)).charindex
            Else
                Engine_Text_Render .dialog, PixelOffsetX + 14 - CInt(Engine_Text_Width(.dialog, True) / 2), PixelY + .Body.HeadOffset.y - Engine_Text_Height(.dialog, True), temp_array, 1, True, MapData(rrX(x), rrY(y)).charindex
...
Sub Char_Render(ByVal charindex As Long, ByVal PixelOffsetX As Integer, ByVal PixelOffsetY As Integer, ByVal x As Integer, ByVal y As Integer)
    ...
    With charlist(charindex)
        ...
        If .Body.Walk(.Heading).GrhIndex Then
            ...
            Else
                If .Muerto Then
                    Call Copy_RGBAList_WithAlpha(Color, MapData(rrX(x), rrY(y)).light_value, 150)
                Else
                    Call Copy_RGBAList(Color, MapData(rrX(x), rrY(y)).light_value)
                End If
                ...
                If .esNpc Then
                    If Abs(tX - .Pos.x) < 1 And tY - .Pos.y < 1 And .Pos.y - tY < 2 Then
                        ...
                        If .UserMaxHp > 0 Then
                            ...
                            Call Draw_Grh(TempGrh, PixelOffsetX + 1 + .Body.BodyOffset.x, PixelOffsetY + 10 + .Body.BodyOffset.y, 1, 0, TempColor)
        ...
        'desactivo temporalmente little
                'Call Draw_Grh(.Head.Head(.Heading), TextureX + .Body.HeadOffset.x, TextureY + OffHead, 1, 0, COLOR_WHITE)
                'If .Casco.Head(.Heading).GrhIndex Then Call Draw_Grh(.Casco.Head(.Heading), TextureX + .Body.HeadOffset.x, TextureY + OffHead, 1, 0, COLOR_WHITE)
                'If .Escudo.ShieldWalk(.Heading).GrhIndex Then Call Draw_Grh(.Escudo.ShieldWalk(.Heading), TextureX, TextureY + OffArma, 1, 1, COLOR_WHITE)
                'If .Arma.WeaponWalk(.Heading).GrhIndex Then Call Draw_Grh(.Arma.WeaponWalk(.Heading), TextureX, TextureY + OffArma, 1, 1, COLOR_WHITE)
                ...
                Select Case .Heading
                    ...
                    Case E_Heading.NORTH 'ESCUDO Y ARMA ESCONDIDA
                        Call Draw_Grh(.Head.Head(.Heading), TextureX + .Body.HeadOffset.x, TextureY + OffHead, 1, 0, COLOR_WHITE)
                        If .Casco.Head(.Heading).GrhIndex Then Call Draw_Grh(.Casco.Head(.Heading), TextureX + .Body.HeadOffset.x, TextureY + OffHead, 1, 0, COLOR_WHITE)
                        If .Escudo.ShieldWalk(.Heading).GrhIndex Then Call Draw_Grh(.Escudo.ShieldWalk(.Heading), TextureX, TextureY + OffArma, 1, 1, COLOR_WHITE)
                        If .Arma.WeaponWalk(.Heading).GrhIndex Then Call Draw_Grh(.Arma.WeaponWalk(.Heading), TextureX, TextureY + OffArma, 1, 1, COLOR_WHITE)
                    ...
                    Case E_Heading.WEST 'arma cuerpo escudo
                    If .Arma.WeaponWalk(.Heading).GrhIndex Then Call Draw_Grh(.Arma.WeaponWalk(.Heading), TextureX, TextureY + OffArma, 1, 1, COLOR_WHITE)
                        Call Draw_Grh_Breathing(.Body.Walk(.Heading), TextureX, TextureY, 1, 1, COLOR_WHITE, ease)
                        Call Draw_Grh(.Head.Head(.Heading), TextureX + .Body.HeadOffset.x, TextureY + OffHead, 1, 0, COLOR_WHITE)
                        If .Casco.Head(.Heading).GrhIndex Then Call Draw_Grh(.Casco.Head(.Heading), TextureX + .Body.HeadOffset.x, TextureY + OffHead, 1, 0, COLOR_WHITE)
            ...
            Else
                ...
                Call Draw_Grh(.Body.Walk(.Heading), PixelOffsetX + .Body.BodyOffset.x, PixelOffsetY + .Body.BodyOffset.y, 1, 1, Color)
            End If
    
            'Draw name over head
            
          '  If Nombres Then
            'Nombres = Not MapData(rrX(charlist(charindex).Pos.x), rrY(charlist(charindex).Pos.y)).zone.OcultarNombre
            
           ' If UserCharIndex > 0 Then
           '     If Sound.MusicActual <> MapData(rrX(charlist(UserCharIndex).Pos.x), rrY(charlist(UserCharIndex).Pos.y)).zone.Musica And MapData(rrX(charlist(UserCharIndex).Pos.x), rrY(charlist(UserCharIndex).Pos.y)).zone.Musica > 0 Then
           '         Sound.Music_Stop
           '         Sound.Music_Load MapData(rrX(charlist(UserCharIndex).Pos.x), rrY(charlist(UserCharIndex).Pos.y)).zone.Musica, Sound.VolumenActualMusicMax
           '         Sound.Fading = 100
           '         Sound.Music_Play
           '     ElseIf MapData(rrX(charlist(UserCharIndex).Pos.x), rrY(charlist(UserCharIndex).Pos.y)).zone.Musica = 0 And Sound.MusicActual <> MapDat.music_numberLow Then
           '         Sound.Music_Stop
           ...
           ' End If
            ...
            If Nombres And Len(.nombre) > 0 And MostrarNombre Then
                ...
                Factor = MapData(rrX(x), rrY(y)).light_value(0).r / 255
                ...
                If .banderaIndex > 0 And .teamCaptura > 0 Then
                    ...
                    Call Draw_Grh(flag, PixelOffsetX + 1 + .Body.BodyOffset.x, PixelOffsetY - 45 + .Body.BodyOffset.y, 1, 0, Color, True)
                End If
        ...
        If Nombres And Len(.nombre) > 0 And MostrarNombre And .tipoUsuario > 0 Then
            ...
            Call Draw_Grh(StarGrh, PixelOffsetX + 1 + .Body.BodyOffset.x + (Engine_Text_Width(.nombre, True) / 2) + 8, PixelOffsetY + 20 + .Body.BodyOffset.y, 1, 0, Color, False)
        End If
        ...
        If .BarTime < .MaxBarTime Then
            ...
            Call Draw_Grh(TempGrh, PixelOffsetX + 1 + .Body.BodyOffset.x, PixelOffsetY - 55 + .Body.BodyOffset.y, 1, 0, Color, False)
...
Public Sub SetMapFx(ByVal x As Integer, ByVal y As Integer, ByVal fX As Integer, ByVal Loops As Integer)
    '***************************************************
    'Author: Juan Martín Sotuyo Dodero (Maraxus)
    'Last Modify Date: 12/03/04
...
Public Sub Grh_Render_Advance(ByRef grh As grh, ByVal screen_x As Integer, ByVal screen_y As Integer, ByVal Height As Integer, ByVal Width As Integer, ByRef rgb_list() As RGBA, Optional ByVal h_center As Boolean = True, Optional ByVal v_center As Boolean = True, Optional ByVal alpha_blend As Boolean = False)
    ...
    'Center Grh over X, Y pos
    If h_center And GrhData(grh_index).TileWidth <> 1 Then
        screen_x = screen_x - Int(GrhData(grh_index).TileWidth * (32 \ 2)) + 32 \ 2

    End If
    
    If v_center And GrhData(grh_index).TileHeight <> 1 Then
...
Private Sub Renderizar_Aura(ByVal aura_index As String, ByVal x As Integer, ByVal y As Integer, ByVal map_x As Integer, ByVal map_y As Integer, Optional ByVal userIndex As Long = 0)
    ...
    If giro > 0 And userIndex > 0 Then
        Call Draw_Grh(aura_grh, x, y + 30, 1, 0, rgb_list(), True, charlist(userIndex).AuraAngle)
    Else
        Call Draw_Grh(aura_grh, x, y + 30, 1, 0, rgb_list(), True)
...
'    Dim Texture As Direct3DTexture8
'    Dim TextureWidth As Long, TextureHeight As Long
'    Set Texture = SurfaceDB.GetTexture(RESOURCES_PATH & "/Interface/es_ventana-papiro.bmp", TextureWidth, TextureHeight)
'    With SpriteBatch
'        Call .SetTexture(Texture)
'        Call .SetAlpha(True)
'        Call .Draw(50, 50, 925, 675, COLOR_WHITE)
'        Call .Flush
'    End With

    Call Engine_Draw_Box(0, 0, frmConnect.ScaleWidth, frmConnect.ScaleHeight, RGBA_From_Comp(0, 255, 0, 40))
...
'    End Select
    ...
    If intro = 1 Then
        Draw_Grh BodyData(770).Walk(3), 490, 303, 1, 0, COLOR_WHITE
        Draw_Grh HeadData(117).Head(3), 490, 269, 1, 0, COLOR_WHITE
            
        Draw_Grh CascoAnimData(5).Head(3), 490, 264, 1, 0, COLOR_WHITE
        Draw_Grh WeaponAnimData(3).WeaponWalk(3), 490, 303, 1, 0, COLOR_WHITE
        Engine_Text_Render "Argentum", 480, 337, ColorGM, 1
        Engine_Text_Render "<World>", 483, 352, ColorGM, 1
    ...
    Engine_Text_Render TextAsistente, 510 - Engine_Text_Width(TextAsistente, True, 1) / 2, 257 - Engine_Text_Height(TextAsistente, True) + TextEfectAsistente, textcolorAsistente, 1, True, , 200
...
Public Sub RenderPjsCuenta()
    ...
    Call Draw_Grh(grh, 0, 0, 0, 0, TempColor, False)
...
Public Function Effect_BeginXY(ByVal Fx_Index As Integer, ByVal Bind_Speed As Single, ByVal x As Single, ByVal y As Single, ByVal Destinox As Integer, ByVal Destinoy As Integer, Optional ByVal explosion_FX_Index As Integer = -1, Optional ByVal explosion_FX_Loops As Integer = -1, Optional ByVal Emisor As Integer = 1, Optional ByVal wav As Integer = 1, Optional ByVal fX As Integer = 0) As Integer
    '
    ' @ Inicia un nuevo efecto y devuelve el index.
    ...
    With Effect(effect_Index)
        ...
        If (.Now_X <> 0) And (.Now_Y <> 0) Then
            ...
            If (.Now_X = .Viaje_X) And (.Now_Y = .Viaje_Y) Then
                ...
                If (.End_Effect <> 0) And .DestinoChar = 0 Then
                    MapData(rrX(.DestX), rrY(.Desyy)).particle_group = 0
...
Public Sub Draw_Grh_ItemInWater(ByRef grh As grh, ByVal x As Integer, ByVal y As Integer, ByVal center As Byte, ByVal animate As Byte, ByRef rgb_list() As RGBA, Optional ByVal Alpha As Boolean = False, Optional ByVal map_x As Integer = 1, Optional ByVal map_y As Integer = 1, Optional ByVal Angle As Single)
...
Public Sub Draw_Grh_Precalculated(ByVal x As Integer, ByVal y As Integer, ByRef grh As grh, ByRef rgb_list() As RGBA, ByVal EsAgua As Boolean, ByVal EsLava As Boolean, ByVal mapX As Integer, ByVal mapY As Integer, ByVal MinX As Integer, ByVal MaxX As Integer, ByVal MinY As Integer, ByVal MaxY As Integer)
    ...
    With GrhData(CurrentGrhIndex)
        ...
        If EsAgua Then
            If mapY > MinY And mapY > 1 Then Top = (MapData(rrX(mapX), rrY(mapY - 1)).Blocked And FLAG_AGUA) * INV_FLAG_AGUA
            If mapX < MaxX And mapX < MapSize.Width Then Right = (MapData(rrX(mapX + 1), rrY(mapY)).Blocked And FLAG_AGUA) * INV_FLAG_AGUA
            If mapY < MaxY And mapY < MapSize.Height Then Bottom = (MapData(rrX(mapX), rrY(mapY + 1)).Blocked And FLAG_AGUA) * INV_FLAG_AGUA
            If mapX > MinX And mapX > 1 Then Left = (MapData(rrX(mapX - 1), rrY(mapY)).Blocked And FLAG_AGUA) * INV_FLAG_AGUA
        
            Call SpriteBatch.DrawWater(x, y, TilePixelWidth, TilePixelHeight, rgb_list, .Tx1, .Ty1, .Tx2, .Ty2, mapX, mapY, Top, Right, Bottom, Left)
        
        ElseIf EsLava Then
            If mapY > MinY And mapY > 1 Then Top = (MapData(rrX(mapX), rrY(mapY - 1)).Blocked And FLAG_LAVA) * INV_FLAG_LAVA
            If mapX < MaxX And mapX < MapSize.Width Then Right = (MapData(rrX(mapX + 1), rrY(mapY)).Blocked And FLAG_LAVA) * INV_FLAG_LAVA
            If mapY < MaxY And mapY < MapSize.Height Then Bottom = (MapData(rrX(mapX), rrY(mapY + 1)).Blocked And FLAG_LAVA) * INV_FLAG_LAVA
            If mapX > MinX And mapX > 1 Then Left = (MapData(rrX(mapX - 1), rrY(mapY)).Blocked And FLAG_LAVA) * INV_FLAG_LAVA
        
            Call SpriteBatch.DrawLava(x, y, TilePixelWidth, TilePixelHeight, rgb_list, .Tx1, .Ty1, .Tx2, .Ty2, mapX, mapY, Top, Right, Bottom, Left)
        
        Else
             Call SpriteBatch.Draw(x, y, TilePixelWidth, TilePixelHeight, rgb_list, .Tx1, .Ty1, .Tx2, .Ty2)
...
Public Sub ResetRenderTarget()
    Call DirectDevice.SetRenderTarget(pBackbuffer, pZBuffer, 0)
End Sub
