...
Public Sub Engine_Text_Render_In_Console(Texto As String, ByVal x As Integer, ByVal y As Integer, ByRef text_color() As RGBA, Optional ByVal status As Integer = -1, Optional ByVal font_index As Integer = 1, Optional multi_line As Boolean = False, Optional charindex As Integer = 0, Optional ByVal Alpha As Byte = 255, Optional ByVal priv As Integer = -1)
    
    On Error GoTo Engine_Text_Render_In_Console_Err
    ...
    End If
     ...
     Select Case status
        Case 0 'criminal
            Call RGBAList(console_color1, 225, 0, 0, A)
            Call RGBAList(console_color2, 255, 255, 255, A)
        Case 1 'ciuda
            Call RGBAList(console_color1, 0, 128, 255, A)
            Call RGBAList(console_color2, 255, 255, 255, A)
        Case 2 'legión oscura
            Call RGBAList(console_color1, 155, 0, 0, A)
            Call RGBAList(console_color2, 255, 255, 255, A)
        Case 3 'armada real
            Call RGBAList(console_color1, 0, 175, 255, A)
            Call RGBAList(console_color2, 255, 255, 255, A)
        Case 4 'Legión
            Call RGBAList(console_color1, 155, 0, 0, A)
            Call RGBAList(console_color2, 255, 255, 255, A)
        Case 5 'Consejo
            Call RGBAList(console_color1, 22, 239, 253, A)
            Call RGBAList(console_color2, 255, 255, 255, A)
        Case 20 'aviso conectado
            Call RGBAList(console_color1, 10, 182, 70, A)
            Call RGBAList(console_color2, 10, 182, 70, A)
        Case 21 'aviso solicitud
            Call RGBAList(console_color1, 255, 255, 0, A)
            Call RGBAList(console_color2, 10, 182, 70, A)
        Case 22 'lider
            Call RGBAList(console_color1, 222, 194, 112, A)
            Call RGBAList(console_color2, 255, 255, 255, A)
        Case 23 'aviso desconectado
            Call RGBAList(console_color1, 175, 0, 0, A)
            Call RGBAList(console_color2, 175, 0, 0, A)
    End Select
    
     If priv > 0 Then
        Call RGBAList(console_color1, ColoresPJ(priv).R, ColoresPJ(priv).G, ColoresPJ(priv).B, A)
        Call RGBAList(console_color2, 255, 255, 255, A)
    End If
        ...
        For A = 1 To Len(Texto)
            ...
            graf.GrhIndex = Fuentes(font_index).Caracteres(B)
                 If graf.GrhIndex > 12 Then
                    ...
                    If font_index <> 3 Then
                        Call Draw_GrhFont(graf.GrhIndex, (x + d) + 1, y + 11, Sombra())
                    End If
                    
                    If (status >= 0 And status <= 5) Or (status >= 20 And status <= 23) Then
                        If separador Then
                            Call Draw_GrhFont(graf.GrhIndex, (x + d), y + 10, console_color2)
                        Else
                            Call Draw_GrhFont(graf.GrhIndex, (x + d), y + 10, console_color1)
                        End If
                    Else
                        Call Draw_GrhFont(graf.GrhIndex, (x + d), y + 10, text_color)
                    End If
                    d = d + GrhData(GrhData(graf.GrhIndex).Frames(1)).pixelWidth
                End If
                If B = 93 Then separador = True
...
Engine_Text_Render_In_Console_Err:
    Call RegistrarError(err.Number, err.Description, "Graficos_Textos.Engine_Text_Render_In_Console", Erl)
    Resume Next
