...
Sub General_Set_Connect()
    ...
    If CuentaRecordada.nombre <> "" Then
        Call TextoAlAsistente("¡Bienvenido de nuevo! ¡Disfruta tu viaje por ArgentumWorld!") ' hay que poner 20 aniversario
    Else
        Call TextoAlAsistente("¡Bienvenido a ArgentumWorld! ¿Ya tenes tu cuenta? Logea! sino, toca sobre Cuenta para crearte una.") ' hay que poner 20 aniversario
...
Public Sub CargarCuentasGuardadas()
    ...
    Dim Arch As String
        Arch = RESOURCES_PATH & "/OUTPUT/Cuenta.ini"
...
Public Sub GuardarCuenta(ByVal Name As String, ByVal Password As String)
    ...
    Dim Archivo As String
        Archivo = RESOURCES_PATH & "/OUTPUT/Cuenta.ini"
...
Public Function IntervaloPermiteLLamadaClan() As Boolean
    ...
    If FrameTime - Intervalos.LLamadaClan >= CONST_INTERVALO_LLAMADACLAN Then
        ...
        '  Call AddToConsole(  "Usar OK.", 255, 0, 0, True, False, False)
        Intervalos.LLamadaClan = FrameTime
        IntervaloPermiteLLamadaClan = True
    Else
        IntervaloPermiteLLamadaClan = False

        ' Call AddToConsole(  "Debes aguardar unos instantes para volver a llamar a tu clan.", 255, 0, 0, True, False, False)
    End If
...
Sub CargarOpciones()
    ...
    If FileExist(RESOURCES_PATH & "/OUTPUT/Configuracion.ini", vbArchive) Then
        Call LoadImpAoInit
    Else
    ...
    Call ConfigFile.Initialize(RESOURCES_PATH & "/OUTPUT/Configuracion.ini")
    ...
    value_2 = Val(ConfigFile.GetValue("OPCIONES", "DialogosEnConsola"))
    
    ConsoleDialog.RENDER_DIALOGS_MAX = IIf(value_2 > 15 Or value_2 < 4, 8, value_2)
    ConsoleDialog.RENDER_DIALOGS = ConsoleDialog.RENDER_DIALOGS_MAX
    ...
    ' Minimapa
    Minimap.Opacity = Val(ConfigFile.GetValueOrDefault("MINIMAPA", "Opacidad", Minimap.Opacity))
    Minimap.x = Val(ConfigFile.GetValueOrDefault("MINIMAPA", "X", Minimap.x))
    Minimap.y = Val(ConfigFile.GetValueOrDefault("MINIMAPA", "Y", Minimap.y))
...
Sub GuardarOpciones()
    ...
    Dim Arch As String: Arch = RESOURCES_PATH & "/OUTPUT/" & "Configuracion.ini"
    
    If IPdelServidor <> "127.0.0.1" Then Call WriteVar(Arch, "INIT", "ServerIndex", IPdelServidor & ":" & PuertoDelServidor)
    ...
    Call WriteVar(Arch, "MINIMAPA", "Opacidad", Minimap.Opacity)
    Call WriteVar(Arch, "MINIMAPA", "X", Minimap.x)
    Call WriteVar(Arch, "MINIMAPA", "Y", Minimap.y)
...
Public Sub WriteChatOverHeadInConsole(ByVal charindex As Integer, ByVal ChatText As String, ByVal red As Byte, ByVal green As Byte, ByVal blue As Byte)
    ...
    With charlist(charindex)
        ...
        If LenB(.nombre) <> 0 And LenB(ChatText) > 0 Then
            ChatText = "[" & .nombre & "] " & ChatText
            Call AddToConsole(ChatText, red, green, blue, False, False, False, rtfLeft, .status, .priv)
        End If
...
Public Sub ResetearUserMacro()
    ...
    AddToConsole "Has dejado de trabajar.", 223, 51, 2, 1, 0
...
Public Sub CrearFantasma(ByVal charindex As Integer)
    ...
    'MapData(rrX(charlist(charindex).Pos.x), rrY(charlist(charindex).Pos.y)).CharFantasma.Body.GrhIndex = charlist(charindex).Body.Walk(charlist(charindex).Heading).GrhIndex
    'MapData(rrX(charlist(charindex).Pos.x), rrY(charlist(charindex).Pos.y)).CharFantasma.Head.GrhIndex = charlist(charindex).Head.Head(charlist(charindex).Heading).GrhIndex
    'MapData(rrX(charlist(charindex).Pos.x), rrY(charlist(charindex).Pos.y)).CharFantasma.Arma.GrhIndex = charlist(charindex).Arma.WeaponWalk(charlist(charindex).Heading).GrhIndex
    'MapData(rrX(charlist(charindex).Pos.x), rrY(charlist(charindex).Pos.y)).CharFantasma.Casco.GrhIndex = charlist(charindex).Casco.Head(charlist(charindex).Heading).GrhIndex
    'MapData(rrX(charlist(charindex).Pos.x), rrY(charlist(charindex).Pos.y)).CharFantasma.Escudo.GrhIndex = charlist(charindex).Escudo.ShieldWalk(charlist(charindex).Heading).GrhIndex
    'MapData(rrX(charlist(charindex).Pos.x), rrY(charlist(charindex).Pos.y)).CharFantasma.Body_Aura = charlist(charindex).Body_Aura
    'MapData(rrX(charlist(charindex).Pos.x), rrY(charlist(charindex).Pos.y)).CharFantasma.AlphaB = 255
    'MapData(rrX(charlist(charindex).Pos.x), rrY(charlist(charindex).Pos.y)).CharFantasma.Activo = True
    'MapData(rrX(charlist(charindex).Pos.x), rrY(charlist(charindex).Pos.y)).CharFantasma.OffX = charlist(charindex).Body.HeadOffset.x
    'MapData(rrX(charlist(charindex).Pos.x), rrY(charlist(charindex).Pos.y)).CharFantasma.Offy = charlist(charindex).Body.HeadOffset.y
    'MapData(rrX(charlist(charindex).Pos.x), rrY(charlist(charindex).Pos.y)).CharFantasma.Heading = charlist(charindex).Heading
...
Public Sub PreloadGraphics()
    ...
    #If Compresion = 1 Then
        ...
        If Not Extract_File(Scripts, RESOURCES_PATH & "/OUTPUT/", "preload.ind", Windows_Temp_Dir, ResourcesPassword, False) Then
    ...
    #Else
        PreloadFile = RESOURCES_PATH & "/INIT/Preload.ind"
    #End If
