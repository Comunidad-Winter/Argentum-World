...
Public Type RECTL                    ' used to set/ref boundaries of a rectangle
...
Public Sub CheckKeys()
    ...
    If GetKeyState(vbKeyUp) < 0 Then
        If UserPos.Y < 1 Then Exit Sub ' 15
            ...
            ElseIf WalkMode = False Then
                UserPos.Y = UserPos.Y - 1
            End If
            Call FrmMain.renderer_MouseMove(FrmMain.MouseBoton, FrmMain.MouseShift, FrmMain.MouseX, FrmMain.MouseY)
    ...
    If GetKeyState(vbKeyRight) < 0 Then
        If UserPos.X > MapSize.Width Then Exit Sub ' 82
    ...
    If GetKeyState(vbKeyDown) < 0 Then
        If UserPos.Y > MapSize.Height Then Exit Sub ' 86
            If LegalPos(UserPos.X, UserPos.Y + 1) And WalkMode = True Then
                If dLastWalk + 100 > GetTickCount Then Exit Sub
                UserPos.Y = UserPos.Y + 1
                MoveCharbyPos UserCharIndex, UserPos.X, UserPos.Y
...
Private Sub CargarMapIni()
    ...
    If UserPos.X < 1 Or UserPos.X > MapSize.Width Then
        UserPos.X = 50

    End If

    If UserPos.Y < 1 Or UserPos.Y > MapSize.Height Then
        UserPos.Y = 50
...
Public Sub RefreshAllChars()
    ...
    If MapSize.Width = 0 Or MapSize.Height = 0 Then Exit Sub
    FrmMain.ApuntadorRadar.Move (UserPos.X - 18) * (FrmMain.MiniMap.Width / MapSize.Width), (UserPos.Y - 13) * (FrmMain.MiniMap.Height / MapSize.Height)
